...
- **Platform roles** govern the **whole server** — every guild, every user. That's this page. To avoid clashing with the guild "admin" role, the platform tier below owner is called **operator**.
...
| **Operator** | Manages users, guilds, and roles platform-wide, has cross-guild access (via break-glass), and approves access requests. |
...
A core principle of Initiative is that **no one has a standing back door into guilds they don't belong to** — not even platform operators. When platform staff genuinely need to reach a guild's data (to investigate a problem, say), they use **explicit, time-bound, recorded** access instead. You manage this from **Settings → Access** (the access-requests page).
