...
Initiative keeps two role systems separate, and it's worth knowing which is which:

- **Community roles** (admin / member) govern a single workspace. Covered in [Working with communities](../guides/communities.md).
- **Platform roles** govern the **whole server** — every community, every user. That's this page. To avoid clashing with the community "admin" role, the platform tier below owner is called **operator**.
...
A core principle of Initiative is that **no one has a standing back door into communities they don't belong to** — not even platform operators. When platform staff genuinely need to reach a community's data (to investigate a problem, say), they use **explicit, time-bound, recorded** access instead. You manage this from **Settings → Access** (the access-requests page).
