...
1. **Guild-scoped (content) table** — add/modify the SQLModel, autogenerate the migration (which builds the table in `public`), then **regenerate the guild schema**: `python scripts/gen_guild_schema.py` (updates `guild_schema.sql`; `backfill_guild_schemas()` applies it to `guild_template` + every `guild_<id>` on next boot). Isolation comes from the per-guild **schema + `SET ROLE`** — new guild content tables do **not** get `guild_isolation`/`is_superadmin` RLS policies. Enforce *initiative-scoping* in app code with the [`membership.py`](backend/app/services/membership.py) clause builders (a stale permission row must never grant access), the way existing content endpoints do.
