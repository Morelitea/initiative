...
3. **`access_grants`** is platform-scoped (managed cross-guild) like `users`: its endpoints use `SystemSessionDep` with explicit capability + ownership checks, with `platform_<tier>` policies for the admin queue and an own-row policy for requesters.
