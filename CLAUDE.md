...
Copy `backend/.env.example`, set `DATABASE_URL`, `SECRET_KEY`, and optional `FIRST_OWNER_*` (legacy `FIRST_SUPERUSER_*` names still accepted), then run `alembic upgrade head` (or `python -m app.db.init_db`) so the schema is current and default settings/owner are seeded. The app connects through **three Postgres logins** (see the tenancy/RLS section): `DATABASE_URL` (the provisioning role — migrations + guild provisioning; the least-privilege `app_provisioner`, NOT a superuser; the app warns at boot if it detects a superuser), `DATABASE_URL_APP` (`app_user`, RLS-enforced request path), and `DATABASE_URL_ADMIN` (`app_admin`, the policy-bound system engine); all three point at the same database. A fourth, optional `DATABASE_URL_BOOTSTRAP` (the database owner) is what **creates** those three — see [`app/db/bootstrap.py`](backend/app/db/bootstrap.py); it is opened and disposed at startup, before migrations, and the app verifies rather than applies when it is unset. The SPA reads `VITE_API_URL`; align it with the reverse-proxy host in every environment. If enabling OIDC, ensure `APP_URL` is publicly reachable so computed callback URLs stay valid.

## Tenancy, Database Architecture & RLS

Tenancy is enforced in Postgres by **which role a request assumes** (`SET ROLE`) and by **what the request's standing says**, not by app-layer checks alone. There are two orthogonal dimensions — the per-guild schema/role, and the platform privilege ladder — both backed by real Postgres roles.
...
2. **Initiative** — a user must **not** reach content of an initiative they are not in. This is the **hard isolation boundary** (it keeps sensitive data away from non-cleared members of the *same guild*). Enforced by RLS via the schema's own `initiative_access(initiative_id, user_id, need_write)`.
...
| `SettingsRLSSessionDep` (`get_guild_settings_session`) | `app_user` → `SET ROLE guild_<id>` for a member; `guild_<id>_ro` for a settings-only grant, `guild_<id>_support` beside a `read_write` grant | The community's own configuration and roster — the surface an administrator keeps while its content is closed, and the only session a settings-only grant may serve. Routed by `SettingsContextDep` (`get_guild_settings_context`, the seam established `for_settings`: the sign-in step-up, lifecycle status and age gate govern the work, not the settings). Gate by rung with `SettingsAdminContextDep`; a route that changes something takes `SettingsAdminWriteContextDep` / `SettingsSeatWriteContextDep`, which also ask a grantee for the `read_write` grant beside the rung. |
| `SeatSessionDep` (`get_guild_seat_session`) | `app_user` → `SET ROLE guild_<id>_superadmin` | The seat's configuration routes (sign-in settings and policy, provider connections and their claim rules, the billing handoff). Refuses anyone the standing says does not hold the seat. A route that changes something takes `SeatWriteSessionDep` / `SeatWriteContextDep`: a lent seat writes only beside a `read_write` content grant. |
...
- **operator / owner** (`data.bypass`): self-issue a **break-glass** grant (`POST /access-grants/break-glass`) — created **and** self-approved in one step (the row is the audit trail), short TTL. Break-glass is **a pair of grants, not an admin**: a content grant (`read` for full read-only of the guild, `read_write` to edit *existing* content) and, beside it, a `superadmin` settings grant for the community's own configuration. Neither makes the holder a guild admin — `app.guild_admin` is computed from the membership row and no grant sets it. What a grant reaches is its own two axes: the content level says read or edit-existing; the settings rung says which configuration surfaces — the roster, invites and initiative structure at `admin`, sign-in and billing at `superadmin` — and a rung reads on its own and writes only beside a `read_write` grant. A content grant alone authors nothing and manages no roster. `data.bypass` is the *right to break glass*, not a standing bypass; with it removed, an operator reaches a guild only after clicking through.

**How a grant reaches a request.** The establishment seam is the only reader of `access_grants`: it looks the live rows up, routes into `guild_<id>_ro` (a read grant) or the restricted `guild_<id>_support` role (`read_write`), and its standing statement recomputes `app.pam_read` / `app.pam_write` from those same rows, in the database, so what the ~200 PAM policy legs read was written from a row rather than from a variable. Revoking or expiring a grant changes the next request's standing with no other write.
...
7. **Route a person into a community only through the seam.** `establish_guild_access` (REST reaches it through `GuildContextDep`/`RLSSessionDep`) looks up the membership or grant, routes the session, and runs one statement that computes the reader's **standing** — membership, initiative roles, the admin fact, the tool switches — in the routed schema. That `GuildContext` is what endpoints and services read: it is **passed down** by whoever holds the session (`permissions.require_access`, `resource_access.authorize` and every serializer that reports a level take it as an argument), and an async helper that was handed only the session reads it back with `app.db.session.require_guild_context(session)`. It is stored as the session's replay parameters, so what Python believes and what the policies evaluate are the same values. `for_settings=True` is the community's own configuration and membership surface, which its administrator keeps while its content is closed. `set_rls_context` refuses a `user_id` and a `guild_id` together without one, and `app/api/context_seam_guard_test.py` fails on the shapes that would skip it. A routing carries **no role**: guild admin is a lookup (`app.guild_admin`, computed from the membership row), and trusted maintenance is admitted by the connection's own login (`app_admin`), not by anything a call says.
8. **A sweep, a poller or a lifecycle job routes with `guild_id` alone** — no user, no role. It runs on the system engine, and the policies' system leg names that login.
...
6. **Session-variable constants** (for the `public` shared-table policies and the PAM read path) — use these NULLIF-guarded forms in migration SQL:
   ...
   # The admin fact is a lookup the standing statement made, not a claim a
   # routing carried; a settings grant names its community separately.
   GUILD_ADMIN         = "current_setting('app.guild_admin', true) = 'true'"
   SETTINGS_GUILD_ID   = "NULLIF(current_setting('app.settings_guild_id', true), '')::int"
