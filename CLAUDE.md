...
**Namespaces**: `common`, `auth`, `nav`, `projects`, `tasks`, `documents`, `initiatives`, `settings`, `tags`, `guilds`, `import`, `stats`, `landing`, `errors`, `dates`, `access`, `command`, `counterGroups`, `dashboards`, `guildHome`, `calendars`, `properties`, `queues`, `trash`, `search`, `comments`, `announcements`, `myTools`
...
| `SettingsRLSSessionDep` (`get_guild_settings_session`) | same as `RLSSessionDep`, plus `guild_<id>_support` for a settings-only grant | The community's own configuration and roster — the surface an administrator keeps while its content is closed, and the only `/g/{guild_id}/…` session a settings-only grant may serve. |
| `SeatSessionDep` (`get_guild_seat_session`) | `app_user` → `SET ROLE guild_<id>_superadmin` | The seat's four configuration routes (sign-in settings and policy, provider connections and their claim rules, the billing handoff). Refuses anyone the standing says does not hold the seat. |
