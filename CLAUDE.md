...
- **Guild schemas carry no RLS policies.** Isolation is the schema boundary + `SET ROLE` (the request login role cannot reach any guild schema). *Initiative-scoping* (which initiatives a user sees within a guild) is enforced in app code via the clause builders in [`membership.py`](backend/app/services/membership.py) (`initiative_scope_clause`, `initiative_access_clause`) — the schema-per-guild cutover removed the in-schema RESTRICTIVE policy layer.
