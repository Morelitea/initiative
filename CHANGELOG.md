...
- **Archived work can be brought back out again** — the last release made archiving mean something everywhere: an archived thing, and everything inside it, stops taking edits. What it did not leave was a way out. Archived content reports itself as read-only, which is what turns its edit buttons off — and the button that would have unarchived it read the same thing, so it switched off too. Seven of the eight tools had no archive button at all, and no archived view to find one in, so a document or a queue that had been put away simply left its list with nothing left to click. Every tool now has an Archive section in its settings that both puts it away and takes it back, and every tool list has an Archived view: documents beside their templates, the rest beside the list's own controls, and calendars — which have no list of their own — in the community's tool table. Bringing something back is offered to whoever could have edited it. Something archived along with the thing above it says so instead of offering a button that would not work: bring back the initiative or project it went in with, and it comes back too.
