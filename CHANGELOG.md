...
- **Passkeys** — a passkey is a key held by your phone, your laptop or your password manager that answers a prompt instead of a typed password, and it only ever answers this site. **User settings → Security → Passkeys** is where you register one, give it a name, and take it back when a device goes. The list says when each was added and last used, and which ones a password manager keeps in sync. Registering one asks for your password, as changing it does; on the app the button opens a browser, which is where a passkey is made.
