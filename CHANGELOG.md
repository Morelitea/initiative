...
- New data export engine powers exporting your guild data as importable JSON envelopes or various report formats
  ...
  - Bulk selection exports: select multiple documents, queues, or counter groups and export them all in one go — each entity renders in the chosen format and the download arrives as a single zip (a selection of one stays a plain file). Document selections offer the formats every selected type supports; same-named entities are numbered in the archive rather than overwritten. Large selections run as a background job like any big export.
