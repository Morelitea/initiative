...
- **The front door is for the person who was sent a link** — the landing page talked to gamers about parties and quests, which is not who arrives at it: most people land there because their group already lives here and somebody invited them. It now leads with signing in and with what happens next — open the link, have a look round, you cannot break this — in the help center's voice, and starting a community of your own comes second. It draws one card per tool from the tool registry, links the help center throughout, and has a **Get the app** section that puts the visitor's own device first: an Android button that downloads the app from the release this server's bundle runs on, and the home-screen install for everything else. On a deployment with a billing portal it also shows the plans, read from the portal's own price book rather than written into the app, with a link to the full comparison; a self-hosted install shows none.
