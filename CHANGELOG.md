...
- **Single sign-on records how you signed in** — when you arrive through a single sign-on provider, it can say what it asked of you: which methods you used, how strong it rates the sign-in, and when you actually authenticated. Initiative used to note only which provider it was and discard the rest, so a corporate sign-in that had already asked for a second factor was indistinguishable from one that had not. All three are now kept against the provider that performed the sign-in — separately for each, because one session can span several communities with different providers — and the methods appear in the audit log beside the sign-in. Nothing changes about how you sign in today; this is the record the per-community sign-in requirements are built on.
