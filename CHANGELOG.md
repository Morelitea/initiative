...
- **Wikis** — a new tool, for the things a team knows rather than the things it is doing. A wiki holds **pages**, and pages sit in a tree you build as you go — start a sub-page from any row, and it appears beneath it. Opening a wiki hands the sidebar over to its pages, the way My Messages does with conversations, so the shape of what you have written stays in front of you while you read it. Pages are written in the same editor as a document, so `[[ ]]` links and the `#` picker work exactly as they do everywhere else — and every page shows what links to it. That last part is what makes a wiki more than a folder: link a page from a task, and the page knows. Switch it on per initiative under **Settings → Tools**.
