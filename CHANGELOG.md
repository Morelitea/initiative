...
- **A webhook subscription stops reading as live once its owner isn't** — a subscription delivers what the person who registered it can reach, so one whose owner had left the community, or whose account was closed, delivered nothing while still reporting itself active. There was nothing to notice it by either: delivery stands down before it attempts a request, so no failure was ever counted and `dead_letter_count` stayed at zero, leaving a dead subscription and a working one identical in `GET /webhooks/subscriptions`. Leaving a community now deactivates the subscriptions that person registered there, which is reversible if they come back, and closing an account removes them outright along with the target URL and signing secret they held. Subscriptions in communities the person still belongs to are untouched.
