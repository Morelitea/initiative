...
- **Where sign-in is configured is something you can change** — whether sign-in belongs to the platform or to each community was fixed when the deployment was installed, and the Authentication page showed it as a badge with no way to act on it. It is a choice on that page now. A deployment that sets it in its own configuration keeps what it has until somebody chooses here, so upgrading moves nobody. Switching keeps every provider that is already set up rather than deleting any, and switching back restores them. Before a switch goes through you are told which communities still require a sign-in of their own, and how many accounts sign in only the way it would withdraw.
- **Single sign-on works whichever way sign-in is configured** — an operator's own providers used to stop being offered the moment communities were allowed to configure their own, so a deployment hosting many communities could offer a password and each community's provider and nothing else: no Google, no Microsoft, no Apple for anybody. They are offered in both arrangements now. What a community requires is untouched, because a community's sign-in rule may only ever name a provider of its own — signing in with the operator's Google does not satisfy it.
- **A community's sign-in requirement can always be lifted** — reading a community's requirement, and clearing it, used to depend on how the deployment was configured and on whether that community was entitled to set one. Both are now always available to that community's admins. Setting a requirement is unchanged and still needs both. Lifting one only ever admits more people, so it carries none of the conditions that setting one does.
