...
- **A community can have a security admin** — deciding which identity providers a community signs in through, and what it asks of people entering it, sat with every community admin, alongside the day-to-day of running the place. Those are different jobs. A community can now have a **security admin**: everything an admin reaches plus the sign-in configuration, and an ordinary admin can neither grant that seat nor take it away. Whoever runs the deployment seats the first one; after that a security admin can pass it on, so a community isn't going back to the platform for every change. Most communities will never have one, and nothing changes for those.
