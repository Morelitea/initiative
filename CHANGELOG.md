...
- **A screen left open no longer quietly goes stale.** The live connection behind a community's pages — the one that moves a card when somebody else moves it — could stop carrying without ever reporting itself closed: a laptop that slept, a network that went away mid-flight, a proxy that closes connections it thinks are idle. The tab kept showing what it had, with nothing to prompt it. The server now says something every thirty seconds even when nothing has happened, so a tab can tell a quiet community from a connection that has died and rebuild the one it has lost. A connection that comes back also asks whether anything moved while it was away, and re-reads the community only if something did — so a board left up on a second monitor catches up on its own rather than waiting for somebody to touch it.
