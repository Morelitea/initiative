...
- Guild sign-in requirements are enforced at the database layer and across every surface: the row-security rule guild content defers to also checks that the session satisfied the guild's required provider, so an unsatisfied session sees zero rows even if an application path missed the check; cross-guild "my" views (My Tasks, My Documents, recents, personal trash, calendar) include a restricted guild's content exactly when the session satisfies its requirement; realtime sockets apply the same check at connect and keep re-checking it for the life of the connection; and native media/download tokens inherit the session's sign-in state. Background jobs that act on membership are exempted through an auditable system marker, while long-lived integration credentials (API keys, device tokens, automation) deliberately never satisfy a requirement and are refused by restricted guilds.
...
- Per-guild authentication: on platforms configured for per-guild auth, guild admins get an Authentication tab in guild settings to manage the guild's own OIDC identity providers (with presets, write-only client secrets, and the guild's callback URL for IdP registration) and to require that members reach the guild only with a session signed in through one of them. A session that hasn't satisfied a guild's requirement gets a dialog offering that provider's sign-in and returns to the exact page it was on afterwards — and completing that sign-in upgrades the session in place, so satisfying one guild's requirement never un-satisfies another's. The requirement binds everyone (members, admins, and platform support access alike); to prevent lockouts, an admin can only require a provider their own session has signed in with, and a provider a guild requires can't be deleted until the requirement changes. Outside per-guild auth posture the entire surface is absent. First-time users arriving through a guild IdP are refused until guild-first onboarding ships.
