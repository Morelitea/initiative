...
- **A thread looks like a conversation.** Each person's picture sits beside their messages and the time sits under them, both once per run rather than once per line — and a run ends when the other person speaks *or* when five quiet minutes pass, so two messages an hour apart never share a timestamp. The composer grows as you type: Enter sends, Shift+Enter starts a new line, and the message keeps the shape it was written in.
