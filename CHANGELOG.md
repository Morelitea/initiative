...
- **The mobile app can take the picture.** Every place Initiative asks for an image — your profile picture, a community's icon and banner, a document's featured image, an image inside a document, a file uploaded as a document, a notice's screenshots — now offers **Take a photo** on the phone app, straight into the upload. Before this, a picture that did not exist yet meant leaving Initiative, taking it, saving it, coming back, and finding it in a file picker. The same control offers the photo library beside the camera, and in a browser it is the file field it has always been.
...
- **Busy communities stop stuttering.** When a lot changes at once — an import landing, somebody bulk-editing a board, a burst of comments on a busy thread — the app worked out what to refresh separately for every single item, re-checking everything it had loaded each time. On a big burst that was a third of a second where the page did not respond to anything. It now works out the whole batch first and checks once, so the cost no longer grows with how much happened: the same burst is under a millisecond, and the screen keeps up while it lands.

- **A dashboard widget is now pointed at data by clicking, and shows anything it can draw.** A widget used to name one of eight kinds of data source, each with its own set of boxes to fill in — one for tasks, one for counts of tasks, one for a counter, one for a calendar. There is now one: you pick what to read, which columns to bring back and what to group them by, and the dashboard writes the query. Which widget draws it is a separate choice, so the same reading can be a chart, a table, a total or a timeline, and every widget offers to fill its own slots from the columns you got back. Existing dashboards keep working — every stored widget was converted with this release. Anything the conversion could not express is left as it was and named in the upgrade log for an administrator to re-point.

- **A dashboard widget's query can be written by hand.** The data step now has two tabs. **Build** is what it was — you pick what to read and how to group it, and the dashboard writes the query. **SQL** is the way past it, for the questions clicking cannot describe: write the statement yourself, with the datasets, columns and functions offered as you type and the answer checked as you pause, so a query that will not run says which word has to change before you save it. Writing one by hand is a one-way step for that widget — a statement carries no record of the clicking it did not come from — so the builder steps back and says so, and you can start a new one whenever you like.

- **What a role may open is now settled everywhere, not just on the screens that remembered to ask.** An initiative role says which tools its members may use, and which tools the initiative has switched on at all. Until now sharing could step around that: hand somebody a link to a queue and they could open it even though their role does not include Queues. It cannot any more — a share reaches somebody only if their role already lets them use that tool, which is what sharing always meant to say, and trying to share past that now says so instead of quietly doing nothing. If somebody has lost sight of something they used to open, the fix is their role under the initiative's settings, not the sharing on the item.
...
- **A webhook hears about a poll being answered, and stops hearing about drafts.** Two changes to what a subscription receives. Answering a poll now sends the notice being updated — it was sending nothing at all, because a vote names its voter and a poll may be anonymous; the event says the notice moved and never says who moved it, so an integration can act on a poll without learning anybody's answer. And a notice still held back as a draft now sends nothing until it goes up, along with anything that happens to it in the meantime — sharing it, tagging it. Publication is the event, from whichever of the two places does it.

- **A webhook now says what a change was on, not only what changed.** Every change in a delivered event carries the things it sits inside — a comment names its task and that task's project, a task names its project — each as the same `{type, id}` pair the change itself uses, so an integration can act on the surrounding item without a lookup of its own. Identifiers only, like the rest of the envelope: what any of them currently says is still read back through the API, where access is checked. Existing subscriptions need no change; the field is additive, and it is empty for anything that sits directly in an initiative.
...
- **The bell no longer sits up to five minutes behind.** A tab with a working connection still asked the server every five minutes regardless, to cover a notification raised on one server process never reaching the process holding your window. That cannot happen any more — a notification reaches every process — and on the rare occasion the link between them does go quiet, the server now says so as soon as it is back rather than leaving you to a timer. So a connected tab asks for nothing on a schedule at all, and a tab that cannot hold a connection (a proxy that drops them, a phone with no signal) still falls back to asking every thirty seconds.

- **A read-only look at a community can no longer write anything.** When somebody is given read access to a community they are not a member of — a support request that was approved, say — or when a member's own access is read-only, the session they get is meant to be exactly that: reading, and nothing else. It was, for everything belonging to the community itself. But the shared records the app keeps outside any one community, among them sign-in tokens and saved view preferences, were still open to that session, because it carried the same shared permissions a full member's does. Nothing ever wrote to them by that route; the permission was simply there to have. A read-only session now holds reads and no writes, wherever it looks.

- **A project page could crash instead of correcting its address.** When a project turned out to belong to a different initiative than the link said, the app rewrites the address to the right one — but building that address failed, and the page fell over with *Something went wrong* rather than quietly correcting itself. The same could happen on any entity page reached by a link naming the wrong initiative.

- **Signing out now clears whiteboards held on the device.** A whiteboard keeps a copy of its scene in the browser as you draw, so a refresh or a crash cannot lose work that has not reached the server yet. Those copies stayed put when you signed out — on a shared computer, the next person's browser still held them. Signing out now clears them, along with everything else that goes at the end of a session. Anything already saved is on the server and comes back when you sign in.

- **Turning off notifications for direct messages and reactions.** Both switches, on both channels, went back on the moment the settings page reloaded: the account update accepted them and then wrote a hand-listed set of preferences that these four had never been added to, so the setting was saved nowhere and every message and every reaction still notified. The four are now written like the rest, and a category added later cannot be left out of the list again.

- **Comments now appear as they are written, wherever they are written.** A comment posted on a notice, queue, counter group, calendar or dashboard sat there until something else made the page refetch — two people looking at the same board saw different things until one of them reloaded. Only tasks updated on their own. Every tool's thread now moves the moment a comment lands, along with the comment counts on the cards around it and the community's recent-activity list; the same goes for reactions, which now show up on a comment without a reload. The signal was already being sent for all of them — the app was listening for three kinds of thread and ignoring the rest.

- **A notice appears on the board as it is posted.** A new notice was invisible to everyone else until they reloaded — and so was an edit to one, a pin, a deletion, a reaction, and every vote in a poll, which made a live poll something you had to keep refreshing to watch. Nothing was being sent about notices at all; now every change to one reaches the boards that show it, including a notice that goes up on a schedule while you are looking at the board. A notice still held back as a draft stays silent until it is published.

- **An app installed or set up by another admin appears without a reload.** Installing an app, renaming it, moving where it sits, turning it off, upgrading it, connecting it or removing it changed nothing for anybody else who had Initiative open — their sidebar kept the old list, and the app's own settings page kept saying it still needed configuring, until they reloaded the page. The community was already being told; the app was not listening for it. Every one of those now reaches the sidebar and the settings page as it happens.

- **A document you are writing in no longer stops saving itself.** Anything that made the page re-read the document — somebody commenting on it, a change of its own landing — put the server's copy back into what the page was holding. What you had written stayed on screen, so nothing looked wrong, but the page had stopped counting it as unsaved: autosave skipped it, leaving the page threw nothing away in warning, and the writing went with it. Every kind of document is filled in once when you open it and left alone after that, which is what the drawing canvas already did and what the writing and the spreadsheet did not.

- **A settings page no longer takes back what you were typing.** These forms fill themselves in from the thing you are editing, and refilled themselves whenever a fresh copy of it arrived — which happens for reasons that have nothing to do with you: somebody comments on the document whose settings you have open, somebody else renames it, another part of the screen saves something. A half-written description, an SMTP password, a storage key or an identity provider's details would vanish back to what the server last said, with no way to get them back. They stay now until you save them or leave, and a form nobody is typing in still keeps up with whatever changed.
- **Everything a community shares now updates as it changes, not just some of it.** A document renamed, a queue item moved, a counter set, an event added to a calendar, a dashboard rearranged, a tag renamed, somebody joining an initiative — all of it sat there until you reloaded. Only tasks, comments, projects and notices were ever wired up to say anything, and each of them said it from the screen that made the change, so anything that changed another way said nothing at all. Live updates now come from the record of what changed rather than from each screen remembering to announce itself: every tool is covered, and so is every change that happens without you — a scheduled notice going up, a background job, an import, somebody else's bulk edit.

- **Live updates reach every window on a server running as more than one process.** A change made on one process only ever reached the windows that process happened to be holding, so on a larger deployment two people looking at the same board could see different things indefinitely. Every process now reads the same record, so a change reaches whoever is watching it regardless of which one handled the write.

- **Coming back to a tab catches it up.** A window left in the background stayed exactly as you left it, however long you were away — the live updates a page gets reach it only while it is open and on screen, so a tab you came back to after lunch showed you the morning's board until you reloaded it. Returning to a window now refreshes what it is showing. Anything it was told in the last half minute it keeps, so moving between two windows is not a round of requests every time.
