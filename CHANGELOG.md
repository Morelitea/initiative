...
- **The by-hand database setup now finishes the job** — `python -m app.db.bootstrap --print-sql` prints the SQL for a server whose database logins are made by hand rather than by the app. One step of it, moving ownership of the app's own tables and functions to the new provisioning login, was printed as a query whose results you were meant to read and then run yourself — so piping the script into `psql`, which is what the instructions say to do, silently skipped it. On a server that had been running under a different login the next start then stopped with `must be owner of …`. That step now runs itself, and reports each object it moved. A brand-new database has nothing to move and never saw this.
