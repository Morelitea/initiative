...
- **Messages tell you about themselves more than once** — the bell, the push and the email for a direct message all fired on the first message of a conversation and then went quiet: the line they came from was never marked read, so every later message joined the line already sitting there and announced itself nowhere. Reading a thread now closes its line — on every device you are signed in on, not just the one you read it on — so the next message that arrives is news again. Leaving a conversation takes its line down with it.
