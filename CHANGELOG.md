...
- **The dev data seeder fills a board that has no Backlog column** — a seeded task names the stage it belongs in, and the seeder looked that stage up as if every project had a column for it. Projects created since the default board dropped to three columns have no Backlog, so seeding one died on the first task filed there. Each stage now falls back to the nearest column the project actually has.
