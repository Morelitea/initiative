...
- **Posts: a bulletin board for an initiative.** A new tool, beside Documents and Calendars: a place an initiative says things out loud — a change of plan, a date, a welcome. The board reads newest first, and a manager can pin the ones that matter to the top, optionally with a date the pin stops on, so a notice about Sunday stops shouting on Monday and drops back into the feed by its own age. Posts are written in the full editor, so a notice can carry pictures and live smart chips that go on showing a task's current column long after it was written. Each one is shared, tagged, commented on and reacted to like anything else here, and can have its own comments turned off. A post carries its whole body, so a board fetches a few at a time as you scroll rather than paging.
...
- **Ask a question on a board.** A notice can carry a poll: a question, up to ten choices, and one click to answer. Pick one or pick several; change your mind, or take your answer back, for as long as it is open. You can set a time it closes, keep the numbers hidden until somebody has answered so the early votes do not sway the later ones, or make it anonymous — which shows the totals and never the names. Otherwise a tap opens who chose what, and who has not answered yet: the people the notice was shared with, the author among them, because writing a question does not stop you answering it. Once people have answered, the choices are fixed and anonymity cannot be switched off — everything else about the poll is still yours to reword.

- **Boards read like a feed.** The board now loads five notices at a time and fetches the next five as you reach them, rather than making you click through pages. Only the notices near your screen are kept loaded, so an hour of scrolling costs the same as a minute of it — each one carries a full editor, which is what that is protecting. And every notice is signed: whoever wrote it, above the headline, the way a comment is.

- **Set when a pin stops.** A pin could always carry an end date and nothing in the app could set one, so every pin was forever. Once a notice is pinned, the line saying so now offers the date — put an end on it, move it, or take it off again — and when the date passes the notice drops back into the feed by its own age without anybody having to remember it. Pinning itself is still one click; adding an end date is the optional second one, next to the sentence that describes it.

- **Schedule a post, and tell the people it is for.** A notice can be written now and go up later: set a time and it stays a draft — nobody sees it, it is in no board, count or search result, and nobody is told — until the hour comes and it goes up on its own. Until then it sits on the board of whoever can edit it, marked as not yet posted, with one button to put it up straight away. When a notice does go up, the people it was shared with hear about it: in the bell, and by email and push if they want them, with a new *Posts* switch under notification settings for both. Shared with three people, it interrupts three people — posting to a corner of a large initiative does not ring everyone's bell.
...
- **Act on somebody where you find them.** A menu on a profile, and on every row of My Contacts: message them where a channel is open, ask for one where it is not, connect, ignore — and remove a connection, which confirms first. Ignoring used to mean knowing an account's exact handle and opening Settings, which is not where anybody reaches for it.

- **A contact row opens that person.** Clicking somebody on My Contacts goes to their profile, because that is where everything you might do about them is gathered — messaging them among it. The profile carries the two actions worth a click rather than a click and a read: *Message* where the channel is already open, *Ask to message* where it is not, and *Connect* beside them; a request already sent stays on screen as a spent button rather than vanishing. Everyone on the page wears whatever they have put around their picture, the way they do everywhere else.
...
- **Requests, either way round, where you were sent to answer them.** Anything waiting on an answer sits above the conversations — asks to message *and* asks to connect, since both are the same question to the person reading them and both are answered here. Somebody asking you carries *Accept* and *Decline* on the row; anybody you asked carries *Cancel*, because an ask left off the page looks like one that never happened. The row says which of the two was asked for, and the mark in the sidebar counts the ones only you can move, so it lands on the thing it was about instead of sending you elsewhere to find it.

- **The age question is asked where it stops you.** Messaging is for people aged 13 and over, and an account that has not answered that could previously only answer it on My Contacts. The question is now on the Privacy tab — which is where somebody goes to change who may reach them, and where every control it gates already lives — and it is My Messages itself for an account that has not answered, rather than a page whose every button would refuse.

- **An empty list says which of your own settings emptied it.** Set to Private and nobody can reach you, so there is nothing to list: the conversation list now says so, and offers the two ways out — a connection reaches you by handle whatever your setting says, and one button opens you to your communities. "Nobody yet" is kept for the case it actually describes, which is an absence of people rather than a choice you made.

- **Find somebody the list cannot offer.** A field above the conversations narrows them to a handle as you type — nothing is fetched for it and nobody new appears, it only puts what is already there within reach. Beside it, a plus: the people you share a community with, grouped by community the way My Contacts groups them, each headed with how many there are, and a box that also takes a whole handle, number included, which is the one way to reach an account no roster of yours will ever list. A big community shows the first twenty and grows by *Show more* rather than paging, so the person you had just spotted stays where they were. Somebody who takes no messages is shown like anybody else and simply offers nothing to click.

- **What else you can do about somebody, on their row.** The overflow menu that lives on a profile and on My Contacts is now on every row of the conversation list: go to them, copy their handle, star them, connect or unconnect, ignore. It waits out of sight until the row is under the cursor, and is simply there on a touch screen, which has no hovering to do. The menu is now the whole of what one account can do about another wherever it appears, and each place leaves out only what it already offers beside it — so starring, which used to exist only as a star on a contacts row, is on a profile too.

- **The conversations are cut into sections.** Unread first, then favorites, connections, and everyone else you can reach — each one folds away, and stays folded next time. Anybody with something waiting is read at the top whatever else they are to you, because somebody has spoken and nothing further down is asking for anything. Below that, somebody starred who is also a connection is listed once, under Favorites: starring is a choice you made about that person, and being connected is only how the two of you are linked.
...
- **The account row at the foot of the sidebar is one line, and the menu behind it is your profile.** The status used to sit under your name in a bubble of its own with a presence dot beside it, and the two of them cost the sidebar a whole extra row. Now the picture carries the dot — the way it does everywhere else you appear — and the status reads as a line under your name. The menu that row opens leads with the banner and picture you wear and the handle you go by, with the thought bubble back over the banner where there is room for it, and it is still what you click to change what it says. Under that: *Set status*, and how you appear named by the state you are in rather than by the question — the entry says *Online*. The theme moved in too, as a sun that turns into a moon, and running the place — *Admin Dashboard*, *Platform Settings* — sits in a section of its own, because that is somebody's other hat rather than another thing about their account. On a phone, where there is no hover and no room beside a sheet, the theme and the presence open in place and a *Back* row climbs out again. *My Stats* and *Community Settings* left the menu — both already live where they belong, in the sidebar's own navigation — and *Sign out* is now marked as the way out.

- **User Settings is My Settings.** It sits among *My Profile*, *My Tasks* and the rest, and it was the only one of them naming you in the third person. Renamed in the account menu, in the command centre and on the page itself. The menu's wording settled at the same time: a place you can go to is titled like a page, and a thing you do to your account is written as a sentence.
...
- **Refreshing a board no longer asks whether you want to lose your changes.** The board keeps a "new post" composer ready off-screen, and the guard that protects a half-written notice was arming the browser's own reload warning whether or not anything had been typed. The same was true of the post and task pages.
