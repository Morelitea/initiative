...
- **Dex joins the providers the setup wizard knows by name** — a small identity provider you point at something else, so it is the way to offer a sign-in Initiative does not speak directly. Pick it and the address is already the right shape; the only thing to get right is the path, which is whatever your Dex config calls its issuer.
