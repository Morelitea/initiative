...
- The `PAM_ADMIN_MAX_MINUTES` setting is now `PAM_OPERATOR_MAX_MINUTES`, the longest access grant an operator or owner may issue. A deployment that set the old name should set the new one; the old name is no longer read.
