...
- **A deployment can ask arriving visitors about cookies** — a chooser along the bottom of the page the first time somebody lands: what is essential and has no alternative, and a switch each for analytics and marketing. **Reject optional** and **Accept all** sit side by side at the same size, and everything optional stays off until it is switched on, so ignoring the question grants nothing. It is off until a platform owner turns it on under **Settings → Platform → Branding**, which suits a server with a public front door and leaves one a group reaches by invitation alone. Answers are kept in the browser that gave them and never reach the server; the landing page's footer and **User settings → Privacy** both reopen the chooser to change them. Where an administrator has configured a sign-up spam check, it names the company behind it.
