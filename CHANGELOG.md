...
- **Everyone has a username, and it is what people see.** A username is a name you pick plus a number — `foobar#1234` — with the number added for you and shown quietly beside the name, so nobody has to accept `foobar7` because `foobar` was taken. Existing accounts get their first name as a username, or a made-up one if there is no name to use; anyone whose username was assigned rather than chosen picks their own the next time they sign in. Where someone has not entered a full name, their username is shown instead of their email address.

- **A guild decides whether it shows real names.** New setting under Settings → Guild: off — the default — and members appear by username; on, and full names are shown where people have entered one. A guild listed in the community directory always shows usernames and cannot turn this on. Searching for a member follows the same rule: you can always search a username, and full names only in a guild that shows them. Typing a whole username with its number finds exactly that person.
