...
- **Apps can own what they manage** — what an app creates belongs to the app, so it stays where it is when the person who set the app up leaves, and the app's name shows as its owner. Community admins can hand a member's content, or content nobody owns, to an app that is switched on, placed where that content lives, and allowed to change it. An app given sharing access can change who sees what it may change, the way a person with the same access could. Uninstalling an app leaves its content unowned for an admin to claim, and switching one off does the same until it is switched back on.
- **Apps ask before acting as you** — an app that needs to do something as you, such as comment under your name, asks for that one purpose and you get a notification. Answer from the app's settings: allow reading, allow reading and changing, or decline, and withdraw it any time. An app acting as you reaches only what you can, in the initiatives it is placed in, and never anything only an admin can do. It stops if you leave the initiative or the community.
- **Adding an app asks everything once** — the dialog that adds an app to your community lists what it wants to read and change, in plain sentences, with everything this server allows already ticked. For an app with pages inside initiatives it also asks where it appears (every current initiative, or ones you pick) and which roles can open it there, moderators unless you add more. Only the community's superadmin can add apps.
- **App updates that ask for more wait for you** — a new version of an app that wants to reach something new, or add a page inside initiatives, is no longer applied on its own. The superadmin gets a notification, and **Community settings → Integrations** shows what the version wants, to accept or decline. Declining keeps the version you have until the next one comes out. When a version stops asking for something, the app stops being able to use it from its next token.
...
- **Apps and content from the Initiative registry** — the marketplace now follows the Initiative registry out of the box, bringing its listings, their publishers, and the apps' registrations. Everything it brings is checked against a signing key built into the server before it is used. Turn it off, see when it last updated, or refresh it now under **Settings → Platform → Integrations**. A server without internet access can upload a registry bundle there instead. An app from the registry that runs as a container goes live once you give it an address under **App services**. Listings, apps and publishers you added yourself are never replaced by the registry.
...
- **Registry settings** — `MARKETPLACE_REGISTRY_PUBLIC_KEYS` and `MARKETPLACE_REGISTRY_ENABLED` are no longer read. `MARKETPLACE_REGISTRY_URL` now points at a registry's base and defaults to the Initiative registry, and `MARKETPLACE_REGISTRY_ROOT` replaces the signing key built into the server.
...
- **Who can open an app's pages follows where it is placed** — in each initiative an app is placed in, choose which initiative roles can open it, under **Community settings → Integrations**. Moderators can unless you change it, and community admins can always open every page. Pages an app marks for admins, and pages for the whole community, open only for community admins. The same entry chooses what the app may read and change in your community, from what it asks for.
...
- **App registrations are keys, not secrets** — an app is registered with the listing it speaks for and its public keys, pasted as a key set or published at a `jwks_uri` on the app's own https origin. There is no shared secret and no verify step, and an app is live while its registration and its publisher are switched on and it has keys. Each `APP_SERVICES_CONFIG` entry now needs `listing_uid` and `jwks` or `jwks_uri`; `secret_env` and `APP_SERVICE_VERIFY_INTERVAL_SECONDS` are no longer read. Operators can switch off every app from one publisher under `/api/v1/app-publishers`.
- **Apps call their installation with their installation token** — the `/api/v1/app-service/*` routes are replaced by `/api/v1/app-platform/installation/config`, `…/connections`, `…/connections/{ref}`, `…/config-status` and `…/events`, and an app lists its installs at `/api/v1/app-platform/installations`. The connect flow now hands the app a signed `return_token` in place of `return_url` and `return_sig`. Rebuild apps on the new app kit.
- **Apps act as you only for what you allowed** — the app-wide "Acting as you" switch is gone, and an app no longer acts as a member through it. An app asks for each purpose on its own and you answer each one in the app's settings, where community admins can also end any member's answers. For operators, registrations no longer carry grants: `delegation` and `app_directory` are gone, a `grants` field in `APP_SERVICES_CONFIG` is ignored with a warning, and `/api/v1/app-platform/delegation/exchange` no longer answers.
