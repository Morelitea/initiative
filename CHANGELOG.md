...
- **The security stream opens its own cases** — binding it gave security work somewhere to go, and now something arrives there. Repeated refused sign-ins against one account open a case, so they reach a person as a piece of work with an assignee and a place to write up what was found, rather than only as rows in the audit log. A run that keeps going stays **one** case, carrying a count of how often it has been seen, instead of one case per attempt; close it and it starts again, and a new case opens, so "did this continue after we looked?" has an answer. A case names the account by id and never the address that was typed, since resolving one to a person is a platform screen and reaching anything it holds is still a break-glass grant that records why it was taken. A deployment that has not bound the security stream is unchanged, and none of this can delay or fail a sign-in.
