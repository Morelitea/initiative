...
- **A community says where its people land** — a community could say which sign-ins counted as its own, and then nothing: everybody arriving through them turned up as an ordinary member, and sorting them out afterwards was somebody's afternoon. **Settings → Community → Authentication** now takes rules as well. Name a group your identity provider reports and say where the people in it belong — as a member or an admin of the community, and in one of its initiatives if you want them started somewhere. Two hundred teams no longer queue at whoever runs the deployment to have this typed in for them. A rule reads a provider you already count as yours, because saying what its groups mean is the same sentence as saying its people are yours; it hands out member or admin and never the seat that decides who may enter. Which claim carries the groups stays with whoever runs the deployment, since that is a fact about the provider rather than a decision about your community — and a provider they have not set it on says so on the rule, rather than letting you find out when nobody matches.
- **Answer once for every community** — where a deployment is a single organisation, every one of its communities was configuring the same identity provider and the same tenant, separately. **Settings → Platform → Authentication** now takes that answer once per provider, and a community that has said nothing inherits it: the button appears on its sign-in page and the arrangement applies. A community that wants something different connects to the provider itself, which replaces the inherited answer outright — nothing is merged — and it can start from what it inherited rather than from a blank form. Connecting with the button off is how it declines one. Withdrawing the answer leaves untouched every community that wrote its own.
- **Wikis** — a new tool, for the things a team knows rather than the things it is doing. A wiki is a place rather than a document: it takes the whole screen, its pages take over the sidebar, and the page you are on lists its own headings underneath it so you can move around a long one without scrolling for it. Pages nest — drag one onto another to file it underneath, or to an edge to place it beside — and are written in the same editor as a document, **several people at once**, so `[[ ]]` links and the `#` picker work exactly as they do everywhere else. Beside each page is what it connects to and what connects back, which is the thing that makes a wiki worth keeping: link a page from a task, and the page knows. A page **starts as a draft** — everybody who writes the wiki sees it, greyed and labelled, and nobody else is told it exists — until you press **Publish** in its header, so a half-written page is never something people read. At the foot of every page are the pages either side of it in reading order, because a wiki is written to be read through and not only searched. **A conversation belongs to the page it is about** — a note about the rota sits on the rota, not on the handbook it is filed in, so the speech-bubble in a page's header opens that page's thread. Opening a document borrowed into a wiki opens the document's own. Documents you already have can be **put in a wiki** without being moved or copied: they keep their address, their sharing and their history, and they sit in the list wherever you drag them. **What a wiki is for** is yours to set, from the gear in its header: order pages by hand, by title or by what changed last; start every new page from one you nominate as the template; hold prose to a reading width or let it fill the screen; give it an accent so two are told apart. The page list has an **open/close-all** control over it, for a wiki filed several deep, and the wiki shelf filters **by tag** as well as by name. Switch the tool on per initiative under **Settings → Tools**.
- **The sidebar is as wide as you want it** — a wiki with pages filed three deep needs more room than a list of initiatives does, so the navigation column now has an edge you can drag. It never goes narrower than it is today and never takes more than half the window; double-click the edge to put it back. The width is kept **on the device** rather than on your account, because how much of the screen is navigation is a fact about the screen you are looking at — a laptop and a desktop each keep their own answer.
- **Passkeys** — a passkey is a key held by your phone, your laptop or your password manager. It answers a prompt instead of a typed password, you prove it's you the way you unlock the device, and it only ever answers this site. **User settings → Security → Passkeys** is where you register one, name it, and take it back when a device goes; the list says when each was added and last used, and which ones a password manager keeps in sync. The sign-in screen gains **Sign in with a passkey**, and most browsers offer yours in the email box as well. On the app the button opens your browser for a moment and hands you back signed in. Whoever runs the deployment can withhold passkeys from the ways-in checklist; the ones people hold are kept until they are offered again. A community can insist on one: **Settings → Community → Security** gains **Also require a passkey**, beside the second-factor rule, and a member who signed in another way is asked for theirs where they stand rather than signed out. And once a passkey is in the list, the password is optional: **User settings → Account → Remove password** takes it away, hands over recovery codes if you had none, and signs every other device out. Lose the passkey and a recovery code sets you a new password from the sign-in screen, mail or no mail. The last way into an account can never be removed.
- **Two-factor authentication** — a password was the only thing between anybody and an account. **User settings → Security** sets one up with an authenticator app, and signing in then asks for a code as well, in the browser and in the app both. Turning it on hands you ten **recovery codes**, shown once and good for one sign-in each. Lose the phone and the codes with it and whoever runs your deployment can clear the factor so you can start again — clear it, and nothing more: no screen shows anybody's key or their codes. Deployments that would rather keep that decision elsewhere can withhold the authenticator entirely, from the ways-in checklist in **Settings → Platform → Authentication**; existing setups keep working until it is switched back on. Nobody is asked for anything until they turn it on.
- **A community says which sign-ins are its own** — a community that wanted its members arriving through a particular identity provider had to set the whole thing up itself: the address, the client registered there, the secret. Every community signing in with Google configured Google again, separately, with its own secret to rotate. **People sign in to the deployment now, and a community says which of its sign-ins count as theirs.** An operator says this site can sign people in with Google; a community says *our* members come in through that, and only our Workspace — the half that was missing, because Google will vouch for anybody with a Google account. A community picks a provider by name and says which accounts on it are theirs, and whether those people join the moment they arrive; it sees no address, no client and no secret, so there is nothing left to get wrong on behalf of every one of its members. Nobody signs in twice: one sign-in serves the whole deployment, and what each community made of it is applied when somebody reaches it — so a community that tightens who counts as theirs has that apply to the people already signed in, rather than from their next sign-in. The two per-community grants are unchanged and still the operator's.
- **A community can say how its members get in** — a sign-in requirement could only name one identity provider, which is no use to a community running two. **Settings → Community → Authentication** now also offers **Any of our sign-in providers** — come in through one of ours, we don't mind which — and **Also require a second factor**, which a code from an authenticator app or a recovery code both answer. Ask for one, the other, or both. Nobody is signed out to satisfy a new rule: a member already signed in is asked where they are, and the session carries on with everything it had proved. You can only set a rule your own session already meets, so it is proved before it binds anybody.
- **A community can decline personal API keys** — an API key is made by one person for their own scripts, and it reaches every community they belong to. **Allow personal API keys**, on the same page, turns that off for yours: no key can be made for the community, and one already made stops reaching it — the answer is decided when a key is used, not when it was made. Keys are not cancelled and keep working elsewhere, so switching it back on gives people theirs back.
- **Decide how long people stay signed in** — a session ended after a month unused and never otherwise, so somebody using the app daily stayed signed in indefinitely. **Settings → Platform → Authentication** takes a figure in hours for the longest anybody may go before signing in again. Blank means no limit. A community under a compliance standard can hold itself to a stricter twelve hours, from its own **Settings → Community → Authentication**, whatever the deployment's own figure says. It reaches phones at once — shortening it brings credentials already issued under the new figure — while people signed in on the web keep the terms they signed in under.
- **Choose which ways in your deployment permits** — a password and single sign-on were both simply on. That page now opens with the ways in, one tick each, and you keep whichever you want; at least one that can start a sign-in stays on. Where accounts sign in only one way, the page says how many before you withdraw it and asks you to confirm that number.
- **Direct messages, for more than two people** — a conversation was always a pair. **Start a conversation** now gathers several people into a group. Everybody named is *asked* rather than added: they see who else is on it before answering, and nothing reaches them until they say yes. A group has no name and no owner — it is the people on it, which is also how you find it again — and nobody can remove anybody; you can leave, and somebody who left can be asked again. Messages stay end-to-end encrypted, exactly as between two people.
- **A deployment can decide it does not do direct messages** — **Settings → Platform → Community** has a switch for it. Turn it off and My Messages goes, along with the **Message** and **Connect** buttons on profiles and member lists. Nothing is deleted: conversations, connections and everyone's privacy settings stay as they were, so turning it back on gives people their threads back.
- **Report something, and a community settles it** — **Report** now sits on a comment, a notice, a picture, a profile and a community's directory listing, and where it goes is decided for you: something a community holds reaches that community's moderators, something about an account reaches whoever runs the deployment. You pick what is wrong, add anything they should know, and that is the end of your part. Moderators get a **Moderation** entry at the top of their initiative, which cannot be switched off, so there is never a community with nowhere for reports to land. **Private conversations are not moderated and carry no Report action.**
- **Ask for help, from the sidebar** — the question mark used to open the documentation front page. It now says **Ask for help** and opens the FAQ, or a form where whoever runs your deployment is taking requests. It is there for everyone in a community, not only its admins.
- **Every setting you keep for a community, in one panel** — the operator's Guilds tab had run out of room, with caps and switches squeezed into table columns. Each community now has **Manage**, grouping its settings into Limits, Sign-in and Features. The table goes back to being a table.
- **An assistant can see what your work is connected to** — the Relations panel records what a task is blocked by and what belongs to what, and an assistant connected over MCP could read none of it. Asking what something is linked to is now part of that connection, and it can draw a link too — which it asks you to confirm, like any other change it makes. Unlinking stays in the app.
- **An assistant can read and write a wiki** — a wiki was the one tool an assistant connected over MCP could not see at all, so asking it about something the team had already written down came back with nothing. It can now read a wiki, its page tree, a page, and what links to that page — and it can start a page and edit one, asking you to confirm each change the way it does everywhere else. Filing a page somewhere else in the tree, and deleting one, stay jobs for a person.
- **Making an initiative walks you through it** — the old form put everything on one screen with the tools folded into an accordion marked *Advanced*, so the decision that shapes an initiative was the one nobody opened. It is four questions now: what it is called, what it is made of, who can get in, and what an ordinary member can do. The tools step is a grid of the tools themselves, and it will not let you build an initiative with nothing in it. New communities start with no initiative at all — the home page asks you to name the first one after whatever you are actually doing, instead of handing you a folder called *Default Initiative*.
- **A comment shows you what it will look like before you post it** — comments and task descriptions sit in one composer with **Write** and **Preview** tabs, and a row of buttons for the syntax nobody remembers, with ⌘B, ⌘I and ⌘E from the keyboard. Lists carry themselves on: Enter under a bullet makes the next one, Enter on an empty one ends the list.
- **Wikis and galleries travel now** — a backup carried everything an initiative had except the two things hardest to retype: what it had written down, and its pictures. Neither could be handed to another initiative either, the way a project or a document could. Both are in a backup now, and both import. A wiki arrives with its pages still filed where you filed them, anything still a draft still a draft, and whichever page you made its home still its home. A gallery arrives with its pictures rather than a list of what they were called. A picture whose file did not make the trip is left out and counted in the report instead of becoming a tile that never loads.
- **The front door is for the person who was sent a link** — the landing page talked to gamers about parties and quests, which is not who arrives at it. It now leads with signing in and what happens next, shows what each tool is for, and puts your own device first in **Get the app**. Where a deployment sells hosted plans it shows them; a self-hosted install shows none.

### Changed

- **What a community may decide for itself is two switches, not three nested ticks** — an operator granting a community control of its own sign-in worked through a ladder: a master tick that opened the tab, with two more beneath it that stayed greyed out until it was on. The master also quietly bundled personal API keys and the session limit, so a community that only wanted single sign-on got the compliance knobs too. It's two independent switches now, in the community's **Manage** panel under **What it decides for itself**: **Its own sign-in** (connect providers, place people, require arrival through one) and **Its own security standard** (refuse API keys, shorten sessions). Neither needs the other. Requiring a sign-in is no longer a separate grant — a community that can already make somebody its admin from a group claim holds the larger power, so the smaller one comes with it. Communities that held the old master keep both switches; communities an operator had explicitly withdrawn from keep nothing.
- **A community's Authentication tab is now Security, and AI and Apps are one tab** — the Authentication tab held single sign-on beside things that aren't sign-in at all: API keys, session length. It's **Security** now, in two sections — *Who gets in* and *On what terms* — each appearing only where the operator granted it, and ordered so you connect a provider before you're asked to require one. AI and Apps, both the superadmin's and both about what the community hands to somebody outside it, are one **Integrations** tab. Nine tabs become eight.
- **The platform's Authentication page gains a Security tab beside it, and AI and App services merge** — how long anybody may stay signed in has moved from **Settings → Platform → Authentication** to a new **Security** tab next to it; the ways in and the provider registry stay where they were. **AI** and **App services** are one **Integrations** tab, AI first. The same two words now mean the same two things at both levels.
- **The platform's dashboard is the Operator dashboard, and an account is one panel** — it was called the Admin dashboard, which is the one word a community has already spent: an admin runs a community, and nothing on this screen is theirs. It is named for the rung it was always for. The users table had also grown a column carrying everybody's real name, which an operator needs for nothing, and a role dropdown wedged into a cell; both are gone, and a row identifies somebody by their handle. Each row now has **Manage**, the way every community already does, holding what there is to decide about one account: their username, their profile picture, whether they are suspended, and where they sit on the platform ladder. Each of those asks for the permission its own action needs, so a moderator opening the panel is offered the first three rather than a fourth control that would turn them down. The same word went stale in the copy around it — screens that said only a **platform admin** could change branding, email or sign-in now name the **owner**, which is the rung that actually holds it, and the one about a community's user limit names the **operator**.
- **Staff reach a community the way everybody else does** — somebody holding the platform's highest permission could break glass into a community and land there as a full admin of it: every document, every setting, the community's lifecycle hold overridden, with none of the limits an ordinary support grant carries. There is one way in now, and it is the same one. A grant says what it reaches and that is what it reaches, whoever holds it. What replaces the old authority is a second kind of grant that says so out loud: **the community's settings** — its billing, its sign-in, its moderation — held at **admin** or **superadmin**, asked for alongside any access to what is inside, or instead of it. Somebody sent to help with a billing question gets the billing screen and nobody's documents; somebody clearing up after an incident asks for both, and each is approved and recorded on its own. Breaking glass still exists for an emergency and still needs no second person, but it now issues two plainly-named grants rather than one unlimited one, and every request, decision and self-issue is written to the audit log with what it was for and how far it went.
- **Communities have a superadmin** — sign-in configuration and billing were open to every admin a community had. Running a community day to day, and deciding who may get into it or what it is billed for, are different jobs, and there is now a seat for the second: **superadmin**, above admin. Whoever starts a community holds it and can pass it on; every admin a community has today becomes one, so nobody loses anything. The **Authentication** tab and the billing portal are theirs, and an ordinary admin no longer sees either. Every community keeps at least one — its last superadmin cannot be demoted, removed, leave, or close their account — unless they are the community's only member, in which case there is nobody to strand and they go freely. That replaces the old "last admin" rule: an ordinary admin can now leave, or close their account, whenever they like.
- **A community's sign-in is its own to configure** — this was several controls answering one question. Whether a community may configure its own sign-in is now settled entirely in its **Manage** panel, instead of a deployment-wide switch stacked on top of a per-community one. **Configuring its own sign-in** is the tick that gives a community the Authentication tab at all — off to start with, which is the answer for most — and **Its own sign-in providers** and **Requiring a sign-in** sit under it. An operator's own providers are offered always, so a deployment hosting many communities still has Google, Microsoft and Apple for everybody. A requirement can always be lifted by the community that set it, whatever it is still entitled to, because lifting one only ever admits more people. And a group rule now belongs to the provider you wrote it for, so two providers stop overwriting each other's grants.
- **Projects start with three columns, and any of them can go** — every project arrived with **Backlog → In Progress → Blocked → Done**. Blocked was a hangover (what a task waits on is a relation now) and most people read Backlog as To Do anyway, so a new project opens on **To Do → In Progress → Done**. Deleting a column with tasks in it no longer demands a replacement in the same stage: pick any column, or leave it and the tasks fall to the project's default. Existing projects keep exactly the columns they have.
- **An initiative can be only the tools it actually uses** — projects and documents were the two tools every initiative had whether it wanted them or not. Both now have a switch under **Initiative settings → Tools**, so a book club can be documents and a calendar with no empty Projects tab. Every initiative that exists keeps both, and new ones still start with both.
...
- **Signing in through a provider whose address ends in a slash** — every attempt was refused, and no combination of settings helped: the address was trimmed of its trailing slash on our side and then matched against the untrimmed one the provider itself sends, so the two never agreed. Authentik is the common case. The provider's own spelling is now kept as it was given.
- **The MCP address works with or without a trailing slash** — pointing an assistant at `…/api/v1/mcp` returned Not Found unless the address ended in a slash, and several clients save it without one — including connectors added in the browser, which give you no way to see why. Both spellings now reach the same place.
...
- **A comment on a notice or a gallery reaches the activity feed** — a community's recent activity is assembled per community, and a comment records which community it is in as it is written. That step had never learned about notices, galleries or wikis, so a comment on one was filed with no community and the feed skipped it: the thread itself read back correctly, and the comment simply never appeared anywhere else. Comments already written are repaired on upgrade.
