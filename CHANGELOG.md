...
- Guild sign-in requirements now cover every surface, not just the per-guild REST routes: cross-guild "my" views (My Tasks, My Documents, recents, personal trash, cross-guild calendar) include a sign-in-restricted guild's content exactly when the session satisfies its requirement; realtime sockets (live updates, document collaboration, counters, queues) apply the same check at connect and keep re-checking it for the life of the connection, so adding a requirement disconnects sockets whose session doesn't meet it; and native media/download tokens inherit the session's sign-in state so images and file downloads behave the same as the page that requested them. Long-lived integration credentials (API keys, device tokens, automation) deliberately do not satisfy sign-in requirements and are refused by restricted guilds.
...
- Guild sign-in requirements now have their UI. On platforms that have enabled per-guild auth, guild admins get an Authentication tab in guild settings to set the requirement (Open, or Require a specific SSO provider) — including a guided "sign in with it first" prompt when their own session hasn't used that provider yet. When a member hits a guild that requires a sign-in their session hasn't done, a dialog offers that provider's sign-in and returns them to the exact page they were on afterwards, instead of a bare error. The tab — and the policy endpoints behind it — exist only on platforms configured for per-guild auth; elsewhere the surface is absent entirely.
- Guilds get their own identity provider registry: on platforms configured for per-guild auth, guild admins manage the guild's OIDC providers directly in the Authentication tab (same presets, write-only secrets, and safeguards as the operator registry), and the guild's sign-in requirement now chooses from the guild's own providers. Signing in with a guild's provider arrives with the guild login flow.
