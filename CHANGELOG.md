...
- **Guilds have a banner.** A guild admin uploads one picture from Settings → Guild → Pictures and it becomes the header of the guild's front page — the guild's own name and description across it — and a strip along the top of its card in the community directory. It is cropped to 4:1 and resized for you, so there is nothing to prepare and no second file to make. A guild that would rather not go and find artwork picks a **banner colour** instead; a guild that sets neither keeps the page it had.
