...
- **Sign-in placement rules on a provider** — operators and owners can write group rules on an identity provider from **Operator dashboard → Sign-in placement**: people in a group land in a community as a member or admin, and optionally in one of its initiatives with a role. A rule places people in a community that accepts the provider's rules on its connection, or everywhere when an owner switches on **Apply placement rules to every community**, and every change to that switch is recorded. For a provider that signs in several directories, such as a Keycloak or Authentik bridge, a rule can name one directory by a claim and value. Communities see the rules that place people in them on their Security tab. **Sign-in placement** also lists every community waiting for somebody to agree the domain or tenant it named, with **Agree** beside each.
- **Bring your work over from Jira and Confluence** — the import wizard on **Community settings → Data** gains a **Jira & Confluence** option that reads your site directly: connect with your site address, email and an API token, tick the Jira projects and Confluence spaces you want — one product or both — and pick the initiative they land in. The token is used only to read what you picked and is deleted as soon as it's been read; reading a big site takes a few minutes, and closing the window and opening the import again picks it back up.
- **Bring your work over from Jira and Confluence** — the import wizard on **Community settings → Data** gains a **Jira & Confluence** option that reads your site directly: connect with your site address, email and an API token, tick the Jira projects and Confluence spaces you want — one product or both — and pick the initiative they land in. The token is used only to read what you picked and is deleted as soon as it's been read; reading a big site takes a few minutes, and closing the window and opening the import again picks it back up. A Confluence space can come from its HTML export too — **Space settings → Export space → HTML** — for a site the server can't reach or nobody wants to hand a token to: upload the zip from the connect step and it's read the same way, everything but its comments.
  - **Jira projects** arrive with their columns, tasks, fields as properties, links between issues, sub-tasks under their parents, comments and images, every other attached file as a document attached to its task, and each sprint as an event on a calendar named after its board. The review lists the fields that will become properties — untick any you'd rather not add.
  - **Confluence spaces** arrive as wikis with their page tree intact, their home page, and labels as tags; folders, and parent pages with nothing of their own to say, become a list of the pages under them. Status lozenges come across as statuses in their own colours, panels as callouts, decision logs as checklists ticked where decided, tabs as a heading over each, embedded videos as embeds, Mermaid code blocks as diagrams, and a children macro as the list of pages beneath. Pictures appear where the page showed them; every other attached file — and any picture a page kept but never showed — becomes a document filed in the wiki under the page it was attached to, and a page's link to it points at that document. A page's footer and inline comments arrive in its comment thread with their replies, each inline one quoting the words it was pinned to; an inline comment marked resolved stays behind, and the review counts it.
  - **Imported together, they arrive joined up.** A page's Jira macros and issue links point at the tasks those issues became, with a smart chip showing each task's live status; an issue's links to pages — in its description, its comments or its list of Confluence pages — point at the wiki pages. A page that mentions an issue from an earlier Jira import is joined up too.
  - Before anything is written you see what's coming and what isn't — a comment only some people could see, a link to something you didn't pick, sprints when the initiative has no calendars, macros with no equivalent like a table of contents or a draw.io diagram — and say who its people are here, each person once.
- **Callouts, statuses, merged cells, drawings and diagrams in documents** — five new things in the text editor, on documents, wiki pages and posts alike.
  - A **callout** is a coloured panel — info, note, tip, success, warning or error — that holds anything a page can: lists, code, even a table.
  - A **status** is a word in a coloured pill — *Done*, *Blocked*, *Waiting on legal* — set by hand, the way a Confluence status lozenge is.
  - **Merged cells**: select cells in a table and merge them into one, or split one back apart.
  - A **drawing** is a small whiteboard inside the page, shown as a picture and opened for editing where it sits.
  - A **diagram** is Mermaid written in a code block and drawn beneath it as you type — readers see just the picture — and any code block set to Mermaid is drawn the same way. Pick any of them from the `/` menu or the toolbar's insert menu. Callouts and merged cells come through every export — Markdown writes them as Obsidian callouts and MultiMarkdown tables, Word and PDF draw them; a drawing survives the Markdown view and a Markdown export as its scene in an `excalidraw` code block, and is left out of Word and PDF — and pages imported from Confluence bring their info, note and warning panels across as callouts rather than quotes.
- **See everywhere you're signed in, and sign out from afar** — **My Settings → Security** used to list only the phones with your account open. It now lists the browsers too: what each one is (*Chrome on macOS*, *Firefox on Windows*), the address it last connected from, when it was last used, and which of them you're reading this on. Sign any of them out from there, or use **Sign out everywhere else** to close the lot in one go and stay where you are — so a browser left open on somebody else's computer no longer takes a password change to deal with.
- **Say how much a notification carries once it leaves the app** — **Settings → Platform → Security** gains three switches, and a community's own **Settings → Security** gains the same three for itself: whether notifications may reach phones, whether they may reach mailboxes, and whether what they say may name the thing they are about. The stricter answer applies, so a community can be quieter than its server but never louder. With details hidden, a push or an email reads “You were mentioned” and the app is where the rest of it is — the bell inside the app always says everything. Switching mobile notifications off also drops the device registrations the server was holding and declines new ones, so nothing goes to Apple or Google at all; devices register again if it is switched back on. Sign-in codes, address confirmations, password resets and notices about your own account are not notifications and keep going either way.
...
- **Watch your server from Prometheus and Grafana** — set `METRICS_TOKEN` and `/api/v1/metrics` answers a Prometheus scrape that carries it as a bearer token: requests by route and status, response times, live connections, database statement times and connections in use, and the number of accounts, communities and signed-in sessions. No series names a community or a person. Left unset, the endpoint isn't there. A starter Grafana dashboard to import is linked from **Configuration → Monitoring with Prometheus**. A database statement that takes longer than half a second also writes a warning to the log, naming the request it served.
- **Put a community on hold** — the status control on the operator's **Guilds** tab has a new setting, **On hold**. Everyone in the community loses access, its administrators included, and it leaves every community list; nothing in it is changed or removed, and setting it back to Active restores it as it was. Its superadmins are told once, by email and in the app, whom to contact — the Support address from **Settings → Intake**, or the general one.
- **Say who to contact** — **Settings → Intake** has a *Who to contact* section: one general address, and an optional one for each kind of work (security, moderation, support, feedback). A notice that tells somebody to get in touch names the address for its kind of work, or the general one when that kind has none — a suspended community's page, for one, now gives its administrators the moderation contact rather than only "whoever runs this server". Nothing needs setting up: with no address saved, notices read as they did.
- **A notice when your community is restricted** — the superadmin of a community that has been made read-only or suspended now sees a notice on opening it, saying what members can and can't do in the meantime. Where the community has help switched on, the notice opens a help request in one step. It shows once per browser session.
- **An assistant can tick off a checklist item** — an assistant connected over MCP could read a task's checklist and reword its lines, but it had no way to mark one done: saving the list keeps every tick exactly as it was, on purpose, so two people editing at once don't undo each other. It can now tick or untick one item, the same way the checkbox in the app does, and it asks you to confirm first.

### Changed

- API: the platform operator endpoints moved from `/api/v1/admin/*` to `/api/v1/operator/*` (for example `GET /api/v1/operator/users` and `PATCH /api/v1/operator/users/{user_id}/platform-role`), and announcement authoring from `/api/v1/announcements/admin/*` to `/api/v1/announcements/operator/*`. The old paths are gone; update any script or integration that calls them. The OpenAPI schemas follow: `AdminUserRead` is `OperatorUserRead`, the `Admin*` request and response shapes are `Operator*`, `AnnouncementAdmin*` is `AnnouncementOperator*`, and `AuthProviderAdminRead` is `AuthProviderOwnerRead`.
- Access-grant windows are no longer settings: the `PAM_*_MINUTES` variables, `PAM_ADMIN_MAX_MINUTES` among them, are not read. A support grant lasts up to 4 hours, a moderator's up to 8, an operator's or owner's up to 24, and break-glass up to 4 (1 by default).
- API: the access-grant approval queue is its own route, `GET /api/v1/access-grants/queue` (requires `access.approve`); `GET /api/v1/access-grants/` lists only your own grants and no longer takes `mine`. `GET /api/v1/access-grants/limits` reports the longest window you may request, and `GET /api/v1/access-grants/break-glass` reports the longest window you may break glass for (`max_duration_minutes`).
- API: a community's entry (`GuildRead`) reports `can_write_settings` — whether you may change the settings your rung reaches — and `GET /api/v1/guilds/{id}` returns that entry to anyone who administers the community's settings, including on a settings grant.
- API: a project's and a document's `initiative` names the initiative (`id`, `name`, `color`) without its member list. Documents now report their `owner`, and projects report `can_configure` — whether you may pin the project, set its default view and manage its filter presets. Read an initiative's members from the initiative itself.
- **A community's superadmin is appointed from inside the community** — the delete-user dialog on **Settings → Platform → Users** no longer offers to promote a member of a community whose only superadmin is the account being deleted. The account holder makes somebody else superadmin from **Community settings → Users**; where they can't, an operator breaks glass into the community and appoints one from the same page, or deletes the community from its own **Danger zone**. The dialog's **Delete community** button is gone: a community is deleted only from inside it, by its superadmin or under a break-glass grant, and **Check again** picks up the resolved seat.
- **Single sign-on rules belong to each community** — the rules on **Settings → Platform → Authentication** that placed people arriving through a provider into a community or an initiative by the groups it reports are gone from that page. A community's superadmin sets them under **Community settings → Security**, in *Where your people land*, for the providers the community has connected. Rules already saved keep placing people at sign-in and are listed there. Each provider's **Groups claim** stays on the platform page.
- API: removed `DELETE /api/v1/admin/initiatives/{initiative_id}`, `GET /api/v1/admin/initiatives/{initiative_id}/members`, `PATCH /api/v1/admin/initiatives/{initiative_id}/members/{user_id}/role`, `PATCH /api/v1/admin/guilds/{guild_id}/members/{user_id}/role`, `GET /api/v1/users/me/initiative-members/{initiative_id}`, `DELETE /api/v1/admin/guilds/{guild_id}`, and `GET`/`POST /api/v1/settings/oidc-mappings`, `PUT`/`DELETE /api/v1/settings/oidc-mappings/{mapping_id}` and `GET /api/v1/settings/oidc-mappings/options`. A community's roles are set with `PATCH /api/v1/guilds/{guild_id}/members/{user_id}` and its rules with `/api/v1/guilds/{guild_id}/auth/rules`. Blockers from `GET /api/v1/operator/users/{user_id}/deletion-eligibility` no longer carry `other_members`.
- **Documents in a wiki can be filed under its pages** — a document added to a wiki sat at the top of it and nowhere else. Drag it onto a page to file it there, like a page; the placing belongs to that wiki, so the same document can sit somewhere different in another. A backup carries where each file sits and a restore puts it back there. Each one in the tree is marked as what it is — a PDF, a spreadsheet, a link to a design tool — rather than all sharing one document icon.
- **Importing from another app belongs to the community** — bringing work over from Todoist, TickTick or Vikunja sat in **My Settings → Import**, which has no community to bring it into: every import from there quietly went nowhere. It now lives with the community's other data, in **Settings → Data**, beside restoring a backup — one wizard where you pick where the work is coming from, drop the file in, say which list and which initiative it lands in, and watch it arrive. Moving a whole community's data is the seat's errand, so that is who it is offered to; importing a single exported file into a tool is unchanged and stays on that tool's own page. Trello and Asana are no longer listed, having never been built.
- **An import keeps what the file actually said** — a project brought in from Todoist, TickTick or Vikunja used to arrive as titles, descriptions, priorities and a checklist, and nothing else: due and start dates, tags, who each task was assigned to, when it was finished, and every comment were dropped on the way in. All of it comes across now. The import also arrives as a new project whose columns are the source's own sections, lists or buckets, instead of asking you to map them onto an existing project's statuses by hand. Todoist leaves completed tasks out of its own export, which the wizard says before you upload rather than after.
- **A suspended community is closed to everyone in it** — suspending a community used to leave its administrators on its settings pages, able to rename it, change its sign-in, hand out invites and leave. Suspension is a time out now: nobody in the community reaches anything in it, settings included, and its membership stays exactly as it was until the suspension is lifted. Members still don't see it at all; administrators see it in their community list with a lock, and opening it says it's suspended and who to contact instead of showing a page of errors. A read-only community is unchanged — its administrators keep running it.
- **A suspended account is in time out** — signing in shows a suspended account why it was suspended, the reason the moderator gave, and whom to contact, and lets it sign out or end its other sessions. Until the suspension is lifted it reaches nothing else: no community, no change to its own profile, and none of the permissions its platform role carries. Nothing is taken away — its communities, its work and its role are all there when the suspension is lifted. Signing in with a password, a passkey, single sign-on or an emailed code works for a suspended account, so its holder can be told.
- API: a project's and a document's `initiative` names the initiative (`id`, `name`, `color`) without its member list. Documents now report their `owner`, and projects report `can_configure` — whether you may pin the project, set its default view and manage its filter presets. Read an initiative's members from the initiative itself.
- **[Initiative cloud only] An automation keeps running when the person who set it up moves on** — a webhook subscription delivered whatever the member who registered it could reach, so it quietly stopped the moment their access changed: leaving the initiative, losing guild admin, being suspended, or closing the account all ended deliveries with nothing to show for it. The subscription still read as active, and because delivery stood down before it ever attempted a request, no failure was recorded and `dead_letter_count` stayed at zero — a dead subscription and a working one were identical in `GET /webhooks/subscriptions`. A subscription is the community's configuration now, and what it delivers is the scope it names: the initiative it was registered against, or the whole community for one a guild admin registers. It outlives whoever set it up, and turning it off is somebody's decision rather than a side effect of an unrelated one. What an automation may *do* with a delivery is unchanged and still decided per request — an app acts under the authorization a member gave it, re-checked on every call, so withdrawing it still takes effect at once.
- **A name is as long as a name** — the names and titles you type had no length of their own. Some stopped at 255 characters, most stopped nowhere at all — a community's name, an initiative's, a project's, a task's, the name on your account — so a pasted page of text became a sidebar entry with nothing that fitted beside it. Every one of them stops at 255 characters now, and the boxes you type them into stop there too.
...
- **A restored backup mentions the right people** — an @mention in a task, a comment, a document, a post or a wiki page was saved in a backup as the account it pointed at on the server it came from, so restoring the backup somewhere else pointed it at whoever had that account there instead — or at nobody. A backup now names each mentioned person by their handle, the restore's *who is who* step asks about them alongside the other people it quotes, and each mention points at whoever you say that is. A mention of somebody you don't place stays their name, linked to nobody. Restoring onto the same server still lands every mention on the same person. A single exported project, document, post or wiki works the same way.
- **Help-request setup points at the right page** — turning on help requests for a community, before the support stream has a project, sent operators to an **Admin › Intake** page that doesn't exist. It now names **Platform settings › Intake**.
- **Group rules place only a community's own people** — a community's sign-in group rules apply only to people its connection counts as its own (the workspace or tenant it narrows the provider to), at sign-in and on the background refresh. Somebody in a group of the same name elsewhere on the same provider is not placed.
- **A page that breaks says so** — when a page failed to draw, the app showed a bare *Something went wrong!* in unstyled text, and a mistyped address showed only *Not Found*; a failure in the app's outer frame left a blank window. Each now gets a proper page: a broken page offers **Try again** and a way back to My Tasks, and keeps the sidebar around it; an address with nothing behind it says so and offers to go back. A page whose code was replaced by a newer release while the tab was open offers a reload instead of an error.
- **Access requests offer the windows this deployment allows** — the durations on **Access requests** were a fixed list per role, so a deployment that set its own `PAM_*_MAX_MINUTES` limits was offered windows the server refused, or not offered ones it allowed. The form now asks the server for the longest window, for a request and for breaking glass alike.
...
- **Signing out on one device leaves your others signed in** — signing out closed every session on the account, so ending one on a phone signed out the laptop and the tablet with it. It now ends the session that asked, and only that one. Signing out everywhere is still what a credential change does: changing or resetting your password, or turning off a second factor, closes every session you have open, as does an operator acting on the account.
- **The last edit before you leave a document reaches everything that reads it** — while a document was open for collaboration, its saved copy caught up only every ten seconds, and moving to another page inside the app dropped whatever came last. The edit itself was kept for anyone who opened the document to edit it, but exports, search and a wiki showing the document read the older copy — a spreadsheet filled in and left straight away showed up blank. The final state is now handed over as you leave, and spreadsheets save their copy every two seconds, as whiteboards do.
- **Columns survive the Markdown view and exports** — switching a document to Markdown and back flattened its columns into one long run of text, and so did exporting it. Columns are now written as Pandoc's fenced divs (`:::: {.columns}`), which read back in exactly as they were; a Markdown export keeps them the same way, and PDF and Word exports lay them out side by side at the widths they had on the page. Table cells also stop gaining an extra space each time the document goes to Markdown and back.
- **Signing in with an emailed code works on a deployment that runs a captcha** — the card that asks for the code never showed the captcha, and the server asks for one before it posts anything, so *Email me a code* answered with "Could not send a code" every time. The captcha now sits on the address step, where it belongs, and a fresh one is asked for after each attempt. Deployments with no captcha configured — most of them — are unchanged.
- **Any document can sit in a wiki, and reads as what it is** — a file, spreadsheet, whiteboard or link opened from a wiki's sidebar came up as an empty text page, and **Add a document** only offered written ones. Each now opens the way it does on its own page — a PDF in its viewer, a spreadsheet in its grid, a whiteboard on its canvas — read-only and across the wiki's full width, and **Add a document** offers every kind, each with its own icon.
- **Imports ask about everyone they name** — the step that asks who each person in an import is here only listed people who had written comments. Somebody who was only ever assigned tasks, or named in a person field like *Reporter* on a task, document or event, was skipped: the import went ahead, told you afterwards it couldn't match them, and left those tasks unassigned and those fields empty. They're all on the list now, for imported projects, documents and calendars and for community backups taken from now on, and whoever you pick is who they become.
- **Restoring a backup keeps replies under what they answer** — a project restored from a backup arrived with every reply as a comment of its own, so a conversation read as a list of unrelated remarks. Replies now land under the comment they answered.
...
- **Export formats have names again** — the community export wizard listed every format as a raw key (`export.formatPdf`, `export.formatCsv`…) instead of *PDF* or *CSV*, and a finished download's toast did the same. The wording had moved and these screens were still looking for it in its old place.
- **A settings grant reaches the settings it was granted for** — somebody sent to help with a community's configuration, by a support grant or by breaking glass, was shown the "you don't have permission" page on every settings tab but two, and the changes that did get through were turned away by the database. The community's settings section now opens at the rung the grant names: **admin** reaches what a community's admin administers — its general settings, its roster and its invites — and **superadmin** reaches those plus its sign-in, its integrations and its billing. A request asks for a content rung and, beside it, an optional settings rung, and each becomes its own grant — so the initiatives and trash tabs aren't offered with a grant that carries no content access.
- **Settings you can only view show as view-only** — somebody helping with a community's configuration on a settings grant, without a read/write grant beside it, was offered every save button, switch and invite on its settings pages, and the server turned each one down. The pages now show those controls disabled with a line saying a read/write grant is what changes them, and **Invite members** leaves the community's menu for them.
- **Access-request notifications open the access queue** — the button on an access-request notification and its email led to a page that doesn't exist. Both now open **Operator dashboard › Access**.
