...
community sees these surfaces yet. Those four entries and the two Changed ones
below them are held here until the grant is offered, rather than announced in
0.70.0 where nobody could act on them.
-->

### Added

- **You can make an account with a passkey and no password at all** — an account could give its password up once it held a key, but a new one still had to be made with one, so a server that had stood passwords down could only take people in through a single sign-on provider. The registration page now offers **Create account with a passkey**, and on a server that does not take passwords it is the only thing it asks for. The key makes the account and signs you in, and the recovery codes it hands you are how you set a password later, so keep them.
- **Breaking glass takes a passkey** — emergency access asks for your second factor at the moment you ask for it, and would only take a code from an authenticator app. An operator who signs in with a passkey was sent off to set up an app as well, to answer a question their key already answers. The form now offers **Break glass with a passkey** beside the code box, and shows whichever of the two you actually have.
- **A support visit that edits a document says so** — a grant is recorded request by request, but editing runs over a live connection rather than a request, so the one thing people most want on the record was the one thing missing. The first time somebody working under a support grant or break-glass changes a document or a wiki page, that is a line of its own naming which one. Once per visit, not once per keystroke.
- **Every request has a name, and the record uses it** — a line in the audit stream and a line in your reverse proxy's log had no way to be matched up, so "what else was this person doing at the time" meant guessing from timestamps. Every response now carries an `X-Request-Id` header, and every audit line carries the same id along with the address the request arrived from and what the browser or app called itself. Behind a proxy, an id your proxy has already assigned is kept rather than replaced, so one request has one name everywhere it is written down.
- **What somebody did with emergency access, not just that they had it** — a break-glass grant or an approved support request produced one line saying access had been given, then nothing for however long it lasted. Every request made while that access is live is now a line of its own: the route, the method, the answer, and which grant allowed it. Knowing an operator held the keys for an hour is not the same as knowing what they opened.
...
- **A community can place people by their identity provider's groups** — name a group your provider reports and say where the people in it belong: member or admin of the community, and optionally in one of its initiatives. Which claim carries the groups stays with whoever runs the deployment.
- **A community can say how its members get in** — require arrival through any of its providers rather than just one, and require a second factor or a passkey alongside it. Nobody is signed out to satisfy a new rule; a member already signed in is asked where they stand. You can only set a rule your own session already meets.
- **A community can decline personal API keys and shorten its sessions** — refuse the keys its members made for their own scripts, and hold members to a stricter session limit than the deployment's. Keys aren't cancelled and keep working elsewhere.
- **A server can require two-factor authentication** — it could be set up by anybody and insisted on by nobody, so a server with an operator signing in on a password alone had no way to change that. **Settings → Platform → Security** now asks the question in two sizes: everybody with a platform role — support, moderators, operators, owners — or everybody with an account. An authenticator app or a passkey both count, and so does a single sign-on that did the second factor on the way in. Nobody is signed out: the next time somebody it covers opens the app they are asked to set one up where they stand, and carry on once they have. The page says how many people that will be before you save. Two things that cannot type in a code — a personal API key, and the app on a phone — stop working for those people until they set a factor up, and start again the moment they do. Where a server asks everybody, communities stop being offered the same question, because it is already answered for their members.
- **A server chooses which ways in it permits** — **Settings → Platform → Authentication** now lists passwords, single sign-on, passkeys and authenticator codes with a tick beside each, so a deployment can stand down the ones it does not want. The last one standing cannot be withdrawn, and a change that would leave accounts with no way to sign in says how many before it goes through.
- **The audit log covers who may reach what, what was configured, and what left** — the record used to be sign-ins, second factors, moderation and break-glass grants, and nothing else. A member added to a community, a document shared with somebody, a sign-in provider swapped out, a whole community exported, an account erased: none of it left a trace. It does now. Memberships and roles in communities and initiatives, every change to sharing, invites, app delegations, sign-in providers and claim rules, a community's own settings and the operator's for it, email, storage and AI configuration, app services, the marketplace catalogue, exports of communities, initiatives and member lists, accounts and communities created and deleted, trash purges, API keys, apps and webhooks, and each time content is sent to an AI provider. Every entry names accounts by id and never copies a value that could hold a secret, an address or a name — a rotated password is recorded as the fact that it was rotated.
- **The audit record is a stream for your log platform** — every entry goes out as one JSON object on the server's standard output, one line per recorded action, accounts named by id and never a secret among them. The served process had never switched that stream on; it does now, with nothing else on it, so whatever already ships your container logs carries the record and a log platform reads it as records rather than text. A new `LOG_LEVEL` setting says how much the application itself says on standard error — `INFO` unless you say otherwise — and it never governs the audit stream.
- **You choose when email reaches you** — **User settings → Notifications → When you hear about it** now asks how often, not just whether: right away, once an hour, once a day at a time you pick, or once a week on a day you pick. Anything held arrives as one message grouped by community and by what it was, rather than a run of separate mails. Things another person addressed to you — a mention, a reply, a task landing on you, an invitation, a direct message — skip the queue unless you say otherwise, and anything you have already read in the app is dropped rather than sent.
- **Pause notifications for the days you pick** — email and mobile stand down across a date range, both days included. Give it an end day and it starts now; give it a start day as well and you can book time off before you go rather than on the way out of the door. A scheduled email that would have landed in the middle of the range waits for the end of it. The bell keeps collecting throughout, and when the pause ends you get one email with everything that happened, minus whatever you read in the meantime. Nothing gets through while it is on, mentions included. Password resets and sign-in mail are unaffected, because those are answers rather than notifications.
- **Nothing interrupts you while you are already using Initiative** — if you are at the keyboard with the app open, the bell has told you, so mobile stays quiet and email waits about ten minutes to see whether you read it yourself. A tab left open since Tuesday does not count as being here. Switchable in the same place.
- **Two endpoints that say whether the server is working** — `/api/v1/healthz` answers for as long as the process is running, and `/api/v1/readyz` checks what it depends on before it answers: its three database logins, file storage, the channel its workers use to reach each other, and wherever rate-limit counters are kept. Readiness refuses when the database is unreachable and reports the rest as degraded, because taking a server out of service does not bring an object store back. If you run Initiative behind a load balancer or in Kubernetes, point liveness at the first and readiness at the second — `/api/v1/version` gives the same answer whatever state the server is in, so a probe on it never notices anything wrong.
- **An assistant can find a thing by its name** — an assistant connected over MCP could only reach something it already had the id of, or guess at a filter, so "open the task about the stale image" meant fetching lists and reading them. It can now run the same search the app's own search page runs — ranked across every tool, comments and tags, narrowable to one kind of thing or one initiative — and it answers under your permissions, so it finds what you would find and nothing else. The two routes behind the search box's own suggestions stay out: those describe a text field's behaviour rather than answer a question.
- **Every tool says what it is connected to** — links were a thing projects, documents and tasks had, and the other six tools did not, which made a relation look like a projects feature rather than the way work refers to work. Calendars and their events, counter groups and counters, dashboards, galleries and their pictures, notices and queues all carry the same panel now, with the same sentence for adding one and wording of their own for what is worth linking from there. A notice reads in two columns like a task does — what it says on the left, its poll and its links on the right, the conversation full width underneath — and an event puts its links between its labels and what it records. A wiki keeps drawing its own, out of the `[[ ]]` in its pages, because a second panel beside that one would be two answers to the same question.
- **Linking one thing to another asks for the thing, not for a category** — adding a relation opened on a dropdown of phrases like "Has as a part", and you had to answer it before the search box would do anything. Nobody starts from the vocabulary; they start from the document they mean. The search box comes first now, offers what you were just looking at before you type, and once you pick something the link is written out as a sentence with both names in it — **This task** *is blocked by* **Order the marquee** — with the middle of it a dropdown holding every wording there ever was. It proposes the mild reading and never guesses at a dependency, because saying one thing is blocked by another is a claim worth making on purpose.
- **A linked thing says what it is doing, not just what it is called** — a list of links was a list of names, so a linked task gave no hint whether it was finished and a linked project none of how far along it was. Each one now carries its current reading: a task's column, an event's date, a counter against its target, and a project as the work in it — **1 / 3**. Projects are a new smart chip too, so `#` a project into a document and the sentence keeps count on its own.
- **A task says what is holding it up, wherever the task is read** — **Blocked by** existed, sat inside a panel somebody had to open, and counted blockers that had been finished for weeks. A task waiting on something now carries a mark on its board card and in every task table, hovering it says what the things actually are, and the count only counts what is unfinished — so the mark clears itself when the last blocker is done, with nobody tidying up. What counts as finished depends on what it is: a task in a Done column, a project whose every task is done, an event that has been and gone, a counter that reached its target. An empty project has not finished anything yet, so it still counts; archived tasks sit it out, so shelving something does not hold a project open forever. A repeating event never counts, having no last time to have passed, and a document is shown as a link but never counted, because nothing about a document says when it is done.

- **A community's export now includes the community** — exporting everything you own gave you the work done inside your initiatives and nothing about the place it was done in: the settings, the tags and the colours on them, who was a member and at what role, which apps were installed. All of it is in the archive now, in a `guild/` folder beside the initiatives. Each initiative also carries its own shape — its roles, what each role could do, who held them, and its property definitions with every option intact rather than only the ones in use — and all of it comes back on import, adding what is missing without touching what is already there. Files nothing currently points at, like an image taken off a page or something uploaded and never placed, come too; that is the one thing a full backup used to drop without saying so.
- **Dashboards export and import like everything else** — they were the one tool left out. A dashboard you built here travels in initiative and community exports now, and comes back on import. One exception, and the archive names it rather than passing over it in silence: a dashboard built on an installed app is not ours to copy into a file, so it is listed as skipped, and the way to have it somewhere else is to install that app there.

- **Very large community exports go to the server, not down your connection** — an export the app hands back is read out through it for as long as your download takes, which is fine for a report and not fine for a whole community. Past about 2 GB the archive is now written to a folder the server administrator has set up — any disk they can write to, including a NAS share, with nothing to sign up for and no internet needed — and the export says it was delivered rather than offering a download. Without such a folder configured, an export that big is refused up front and says what to ask for, instead of running for an hour and producing something nobody can fetch. The export screen tells you which of the two will happen before you start.
- **Sign in with a code sent to your email** — no password, no authenticator, nothing to remember: type your address, read the six-digit code out of your inbox, and you are in. The same box makes an account for somebody who does not have one yet, and confirms their address on the way, so a new person is not sent off to a separate page to pick a password they will forget. **Settings → Platform → Authentication** has the tick for it, and it is off until you turn it on — unlike a passkey or a provider, every account with an address can use this one the moment it is permitted, so it is a decision rather than something an upgrade hands you. It needs email set up, and says so if it is not.
- **Three more sign-in providers the setup wizard knows by name** — Salesforce, JumpCloud and Dex join Google, Microsoft Entra, Okta and the rest in **Settings → Platform → Authentication**. Pick one and it already knows how that provider spells its address, which is the field people get wrong. Dex is the interesting one: it is a small identity provider you point at something else, so it is how a deployment offers a sign-in Initiative does not speak directly. Any other provider that speaks OpenID Connect still works — the named ones just save you finding the right URL in somebody's documentation.

### Changed

- **Deleting your account no longer erases it on the spot** — the danger zone stripped your name, your address and your history out of every community the instant it was confirmed, and there was no way back for you or for anybody else. Your account still disappears for everyone else the moment you delete it, and now nothing is erased for thirty days. **Signing in during that time calls the whole thing off** — your communities, your initiative roles and the documents you own are all exactly where you left them, because nothing was dropped. An operator can also restore an account from **Settings → Platform → Users**, where a deleted one shows a red **Deleted** tag and the day it is erased. The window is the deployment's to set, beside the community one, and leaving it blank means accounts are never erased on a timer. Deactivating is untouched and still means what it meant: somebody taking a break is never erased by a clock. For operators, deleting somebody from the users table now takes the same thirty days, and a permanent deletion asks for consent on its own screen — with the recoverable version offered there, one checkbox away.
- **Deleting a community no longer destroys it on the spot** — the danger zone removed everything the instant it was confirmed: every initiative, project, task, document, member and invite, gone, with a database backup the only way back. A community still disappears for everyone the moment it is deleted, and its own admins can no more undo it than before — but nothing is actually destroyed for ninety days. In that window a platform operator can bring it back from **Settings → Platform → Communities**, where a deleted community now shows a red **Deleted** tag in place of its status control and a **Restore** button under **Manage**, with the date everything is destroyed on. How long that window is belongs to whoever runs the server — **Settings → Platform → Communities** takes a number of days, and leaving it blank means deleted communities are never destroyed at all, which is the answer for a deployment that has undertaken to keep what its members put in it. The date is counted from each deletion, so lengthening the window rescues communities already in it. Restoring asks what the community comes back as — active, read-only or suspended — and, where nobody is left who can run it, who takes it over. Its connections to installed apps are the one thing that does not come back: those end when the community does, because it is the community that authorised them, and its admins reconnect them.
- **The links panel says what it is for** — it was called "Relations" on every page and subtitled with a sentence about connecting things, which described the data and not the job. It is now named for what it sits on, keeps its explanation one hover away instead of under the heading forever, and waits until there is something in it before offering four ways to look at it. The headings read as plain English: **Blocking** rather than "Blocks", **Made up of** rather than "Parts", **Mentioned in** rather than "Referenced by" — which is the backlinks that `#` and `[[ ]]` were already writing for you, under a name that says so.
- **The audit log records where somebody was only when that is the point** — every line had started carrying the network address a request came from and what the browser called itself, including lines about ordinary work: making a document, installing an app, adding somebody to an initiative. That is a location trail on your members for no operational gain, so it is now kept for the two kinds of entry it answers a question about — signing in, and reaching a community you don't belong to. Every other line still names the request it came from, which is what lets you line it up against your other logs, and says nothing about where the person was.
...
- **Sidebar counts leave archived items out** — the number beside each tool in the sidebar counted archived documents, queues, calendars, dashboards, galleries, notices and wikis along with the live ones, while projects already left them out. Every tool now counts what is on its board.
- **Any tool's list can be filtered by tag** — the tag picker in the list filters was offered for wikis alone, and the other lists ignored a tag if one was sent. Queues, counter groups, dashboards and galleries take the same picker now, and every tool's list answers a tag filter the same way.
...
- **The age question belongs to the community, not to the whole platform, and the minimum is now 16** — an account that had not answered was held on a date-of-birth screen with the whole app behind it, including the communities it had been invited to and had been using for months. It now stands in front of one thing: joining a community anyone signed in can find. Every way into one asks — the directory and an invite alike — and a community that has not listed itself asks nobody, whoever brings them in. Answer it and you join; close it and the rest of Initiative carries on exactly as it was. Where nobody is at a keyboard to be asked, such as a group rule from your identity provider, an account that has never been asked is let in and one that answered under the minimum is not — and the community puts the question at its own door the first time they open it, which is the moment somebody is finally there to answer. Until they do, that community is the only thing closed to them.
- **A community holding an under-age member cannot be listed** — a community that has been private until now gathered its members under no age rule, so listing it where anyone can find it is refused while it holds somebody who has told us they are under 16. The settings page names that as the reason, alongside the shelf, adult-content and capacity rules it already gives. Checked when it goes on the shelf and not afterwards, so an ordinary edit to a listed community is never failed over a member's answer.

- **A large community can export all of it** — the whole-community export refused outright past fifty thousand records or 256 MB of files, which is a size a real community reaches, and there was no partial answer to fall back on. The archive is now written to disk as it is produced instead of being held whole in memory, so the ceilings that stood in for that are an order of magnitude higher: half a million records and 10 GB of files.

- **Whole-community exports have a waiting period** — a community's entire content is not something to re-read on a loop, so one can be taken every couple of days rather than on demand. It counts across the community, so a second administrator does not get a fresh turn. Exports of a single initiative, a project or a document are unaffected.
- **Exports can download straight from object storage** — on a deployment using S3-compatible storage, a finished export can now come from the store itself instead of being passed through the application first. Same link, same permissions checked the same way; it just stops a long download from occupying the server for its whole duration. Off until you turn it on with `EXPORT_PRESIGNED_DOWNLOADS`, because your bucket has to allow the app's origin in its CORS rules first.

### Removed

- **Addresses the app stopped using no longer forward** — `/tasks`, `/projects`, `/documents`, `/initiatives`, `/contacts`, `/settings` and a community's `/i` and `/settings/export` were kept as forwarding addresses when those pages moved, some of them for most of a year. A bookmark on one of them now finds nothing; every one of those pages is reachable from the sidebar where it lives.
- **Uploads are no longer relocated on the way up** — a server storing files on disk moved them into per-community folders the first time it booted after 0.53.2, and did nothing on every boot since. Upgrading from **0.53.1 or earlier** now needs one stop at any release from 0.53.2 to 0.70.1 first, so that move still happens; from 0.53.2 onwards there is nothing to do.
- **There is no longer a way to put off making the database logins** — the app refuses to start when `DATABASE_URL` connects as a superuser, and a setting let a server keep booting past that refusal until a date the operator picked. That setting is gone, and a server still relying on it needs its three logins made before it upgrades. The refusal now spells out both ways to do that: set `DATABASE_URL_BOOTSTRAP` and let the app create them on the next start, or run `python -m app.db.bootstrap --print-sql` and apply the SQL as the database owner. Either takes about a minute, and neither touches your data.
...
- **A phone that signed in with a passkey now counts as having signed in with one** — sign in on the app with a passkey and the app was handed a credential that said nothing about how you got it, so a community that requires a passkey turned you away and told you to open a browser. What the sign-in proved now travels with you into the app. It travels once, with that sign-in: an app coming back days later on a credential it has been keeping is not the same as somebody presenting a key, and is asked for one.
- **Scanned PDFs render again, and so do pages a browser had given up on** — the PDF viewer moved to pdf.js 6, which decodes the image formats scanners and fax machines produce in WebAssembly rather than JavaScript. Those modules now ship with the app and are served from it, so a scanned document draws rather than coming up blank, and none of it reaches for the internet on a deployment that has none.
- **A PDF's narrow pages were stretched to match its widest** — a document that mixes single pages with double-page spreads had every page blown up to the same width, so a single page rendered twice the size of the same page inside a spread. The viewer now fits the widest page to the window and draws every other page at that same scale, the way a browser's own PDF viewer does, so the pages keep their relative sizes.
...
- **A container started on an older image than the database says why** — start a version whose migrations stop short of where the database has already been taken, and the app died on an Alembic traceback and restarted into the same one forever: the stack looked like it was running and served nothing, with the reason buried in a container log. It now stops with a message naming what the database is stamped at, the newest migration the image has, and that starting the newer release again is the way back. It also says where to look for a container a half-finished rebuild left behind under a renamed name, which is how a box ends up on the old image after pulling a new one.
...
- **Changing when your email goes out could not be saved** — setting quiet hours, a pause, or a different cadence under **User settings → Notifications** answered with a permissions error, because re-timing the mail already waiting for you ran on the wrong database connection. It saves now, and anything waiting is re-timed against what you chose.
- **A restored backup kept the work and lost the record of it** — every task came back dated the day of the restore, every comment on it was gone, and anything a task was blocked by, part of, or related to was gone with it. A backup now carries when each task and wiki page was written and last touched, what was said on each task, and the links between tasks, and a restore puts all of it back. A file that sat in a wiki sits in that wiki again. An imported comment goes to the person who wrote it: the import matches each name in the archive to an account, and the wizard now has a step where you check those matches and say who the rest are. Leave somebody unmatched and their comments keep the name they arrived with, credited to no account — no avatar, no profile — rather than being put under the name of whoever ran the import. A link whose other end was not in the backup is counted in the report rather than quietly dropped.
