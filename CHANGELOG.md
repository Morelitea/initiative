...
- **A dashboard reports on live work** — a question asked of your tasks answered with all of them, including the ones inside template projects and the ones somebody had archived, so a board of "who is carrying what" dealt out the steps of every template alongside real work. A new widget now starts by leaving out archived rows and templates, written as ordinary filters you can see in the builder and delete when the question really is about archived work. The dashboards that ship with the app — Project health, Team activity, Task flow, Delivery timeline and Event planner — have been updated to ask the same way.

### Fixed

- **The trash stays out of dashboards** — a deleted task could still be counted in a dashboard's figures, for the guild admin and for whoever deleted it, because both are allowed to see the trash where it is managed. A dashboard is not where it is managed, and the two of them seeing different numbers from everybody else on the same board made the numbers mean nothing. Reader-written queries no longer reach deleted rows at all, for anyone. The trash screen itself is unchanged.
- **"Finished, last 30 days" counts again** — the tile appears on four of the shipped dashboards and every one of them failed to load, as did any query measuring a stretch of time. A value the query was told to read as a length of time was being handed over as plain text instead, which the database refused at the moment of asking — late enough that nothing checking these queries beforehand had ever seen it.
