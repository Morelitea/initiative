...
- More PAM grantee 500s: opening AI/guild settings (`/settings/ai/resolved`) crashed because it lazily created a `guild_settings` row, which a grant can't write; it now resolves against transient defaults. And because a read-write grant now reports `write` access, the member/permission management endpoints (project, document, and counter-group member and role-permission management) became reachable and faulted trying to write access-control tables a grant can't touch — these now return a clean 403 (a grant confers content read/write only, never access-control management).
- A PAM grantee got 403s reading task and document comments (the comment access check was pure DAC and a grantee has no permission rows). A grant now satisfies comment access — read to view, read-write to post — and the cross-guild "recent comments" feed includes a grantee's guild instead of filtering it out. A read-only grant still can't post.
