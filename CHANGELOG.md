...
- **Five more sign-in providers the setup wizard knows by name** — GitLab, Gitea and Forgejo, Discord, Salesforce and JumpCloud join Google, Microsoft, Okta and the rest in **Settings → Platform → Authentication**. Pick one and it already knows how that provider spells its address, so the only thing left is registering an app at the other end and pasting two values back. Each has a quickstart in the docs for the bit you do over there. Any other provider that speaks OpenID Connect still works — the named ones just save you finding the right URL in somebody's documentation.
- **An import asks who its people are before it writes their words down** — dropping an exported project on a board imported its comments straight away, and anybody the file quoted who was not already a member here lost their account: the words landed under their old name with no face and no profile. The file now stops for a moment first, lists everybody it quotes with how many comments each of them wrote, and asks who they are on this server — the same step a whole-community restore already had. Answers where the handle is already a member are filled in; leaving a row blank is a real answer, and those comments keep the name they arrived with. It only asks when there is something to ask: a file that quotes nobody, or one where everybody is already here under the same handle, still imports on one click. Who you say somebody is now decides who a task is assigned to as well, not just who wrote a comment — though an assignment still only lands on somebody who is in the initiative it is going into, because being assigned work is a statement about who is working on it now.
- **Notices can be imported from the board that lists them** — every other tool's list page took an exported file; the bulletin board did not, so a notice's own export had nowhere in the app to go back in. The board's overflow menu now offers **Import from a backup** like the rest of them.
