...
- **The API says what you may do** — every tool's read now carries `can` (`edit`, `delete`, `share`, `export`, `unarchive`; projects add `configure`, and a calendar event carries `edit`) in place of `my_permission_level` and `can_unarchive`. An initiative's read carries `can` (`manage`, `moderate`, `view`, `create`), and `GET /initiatives/{id}/my-permissions` is gone. Update any script that calls the API with a key and reads these.
