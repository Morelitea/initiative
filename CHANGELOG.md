...
- **Fresh installs through `app_provisioner` no longer fail on Postgres 15+.** The database init script now hands the `public` schema to `app_provisioner` (Postgres 15 made CREATE on it owner-only) and pre-creates the `app_user`/`app_admin` login roles in superuser context (Postgres 16 lets only BYPASSRLS holders create or alter BYPASSRLS roles). The baseline migration now only syncs those roles' passwords and no longer names `BYPASSRLS` in `ALTER ROLE` when the attribute is already set. `create-provisioner.sql` gains the same `public`-schema ownership handover for existing installs.
