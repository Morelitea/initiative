...
- **An initiative you can't do anything in no longer takes up space in the sidebar.** A role with every tool turned off left the initiative's name in the sidebar with nothing under it, and listed it on the Initiatives page the same way. It is now hidden for the members holding that role — an initiative appears once its role can see at least one tool in it, or it offers an installed app's page. Guild admins, initiative managers, and anyone with a grant are unaffected, and nothing about membership itself changes.
