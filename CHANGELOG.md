...
- **Where operations work lands is set in the community that receives it** — choosing which project each stream (security, moderation, support, feedback) lands in, setting one up from its ready-made project, and pausing or stopping one have moved from **Settings → Platform → Intake** to the operations community's own **Community settings → Intake** tab, where its superadmin sets them up. The tab appears only there, and only to its superadmin. The platform's **Intake** page keeps choosing which community receives operations work and who to contact, shows which streams that community currently receives, and links to its **Intake** tab. Bindings already saved are unchanged.
- API: binding a stream moved to the operations community's own routes, all of which answer 404 in any other community and need its superadmin: `GET /api/v1/g/{guild_id}/intake` (every stream and where it lands), `GET /api/v1/g/{guild_id}/intake/options`, `PUT` and `DELETE /api/v1/g/{guild_id}/intake/{stream}`, and `POST /api/v1/g/{guild_id}/intake/{stream}/blueprint`. Removed `GET /api/v1/settings/intake/options`, `PUT` and `DELETE /api/v1/settings/intake/{stream}` and `POST /api/v1/settings/intake/{stream}/blueprint`. `GET /api/v1/settings/intake` and the `PUT`s beside it no longer return `bindings`; they return `receiving`, the streams that currently have somewhere to land.
