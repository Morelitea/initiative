...
- **Tasks now record when each assignee finished their part.** Being done with your share is not the same as the work being over — you can finish and hand a task to review while it stays open for whoever has it next. Closing a task finishes everyone's part, and pulling it back out of a Done column reopens everyone's; anyone who had already marked their own keeps the time they actually finished.
- **A Focus list at the top of My Tasks.** What is still yours to do right now — work due soon plus anything urgent — with whatever you pin held at the top regardless of its dates. Ticking something off marks *your* part done rather than closing the task, so work you have handed on leaves your list without pretending it is finished — unless you are the only assignee, where your part is the whole task and it closes with you. Nothing vanishes when you tick it: it stays on the list, struck through, until the day turns over, alongside a running "3 of 6 done" count. Pin or unpin from either the list or the task table below it, set the date window and the urgent-work rule from the settings menu, and collapse the whole section if you would rather not have it. Every task matching your settings is shown, so a shorter list comes from a tighter date window rather than a hidden cutoff. The list spans all your guilds and is independent of the table's filters, so narrowing the table never empties it.
