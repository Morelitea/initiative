...
- **Decide how long people stay signed in** — a session ended after a month unused and never otherwise, so somebody using the app daily stayed signed in indefinitely. **Settings → Platform → Authentication** takes a figure in hours for the longest anybody may go before signing in again. Blank means no limit. Communities under a compliance standard can be held to a stricter twelve hours from their **Manage** panel, whatever the deployment's own figure says. It reaches phones at once — shortening it brings credentials already issued under the new figure — while people signed in on the web keep the terms they signed in under.
