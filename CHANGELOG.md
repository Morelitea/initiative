...
- **A community connects to a sign-in provider rather than configuring one** — a community that wanted its members signing in through an identity provider had to set the whole thing up itself: the address, the client registered there, the secret. Every community signing in with Google configured Google again, separately, with its own secret to rotate. **The providers belong to whoever runs the deployment now, and a community connects to one.** An operator says this site can sign people in with Google; a community says *our* members come in through that, and only our Workspace — the half that was missing, because Google will vouch for anybody with a Google account. A community picks its provider by name and says which accounts on it are theirs; it sees no address, no client and no secret. One registered for a single customer is offered to nobody and connected to them alone, so no other community learns it exists: each provider carries **Communities may connect to this**, off to begin with. The two per-community grants are unchanged and still the operator's.
