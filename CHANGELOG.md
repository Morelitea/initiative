...
- Document downloads are now guild-addressed: document ids repeat across per-guild schemas, and the old download URL (a bare id, with no guild header possible from iframes) could 403 or serve another guild's same-id file for users in multiple guilds. Every download URL now carries a required `guild_id` (the app appends it automatically), the document and file-version queries are pinned to that guild so a frozen `public` backup row can never be served, non-members fail closed, and the resolver structurally guarantees a live, correctly-routed document for follow-up queries.
