...
- **A community connects to a sign-in provider rather than configuring one** — a community that wanted its members signing in through its own identity provider had to set the whole thing up itself: the address, the client it registered there, the secret. That is the wrong job to hand somebody whose actual job is running a community, and it meant every community signing in with Google configured Google again, separately, with its own secret to rotate. It also meant the providers on a deployment were spread across as many pages as there were communities. **The providers now belong to whoever runs the deployment, and a community connects to one.** An operator says this site can sign people in with Google; a community says *our* members come in through that, and only our Workspace — which is the part that was missing, because Google will vouch for anybody with a Google account. A community picks its provider by name from the ones on offer and says which accounts on it are theirs; it sees no address, no client and no secret, and there is nothing left for it to misconfigure on behalf of its members. A community bringing its own identity provider works the same way with the operator registering it: added once, offered to nobody, and connected to the one community it belongs to — so no other community learns it exists. Each provider now carries **Communities may connect to this**, off to begin with, which is what keeps one customer's provider out of every other customer's list. The two per-community grants are unchanged and still the operator's: whether a community may connect anything at all, and whether it may insist its members arrive that way.
- **Decide how long people stay signed in** — a session ended when it went unused for a month, and never otherwise: somebody using the app daily stayed signed in indefinitely, which an organisation answering to an auditor cannot say out loud. **Settings → Platform → Authentication** now takes a figure in hours for the longest anybody may go before signing in again, however much they use it. It starts blank, meaning no limit, because a self-hosted deployment is not answering to anybody — and nothing about upgrading shortens a session that is already open. Communities can be held to a stricter one: **Manage** for a community, under Sign-in, has a twelve-hour switch for the ones under a compliance standard, and its members sign in again on that schedule whatever the deployment's own figure says. Being in two such communities is the same twelve hours, not a sum or a race. Two things worth knowing before you set a number. It is the *absolute* limit, separate from being signed out for not using the app, so a figure shorter than that window becomes the only thing that ever ends a session. And it reaches phones at once: a phone holds a longer-lived credential, and shortening the number — or turning on a community's twelve-hour switch — brings the ones already issued under it straight away, so somebody whose phone signed in last week is asked for their password again. People signed in on the web keep the terms they signed in under. Lengthening the limit, or clearing it, signs nobody out, and a phone still signed in goes back to the longer window from its next renewal — one that was already signed out has to sign in again.
- **A community can decline personal API keys** — an API key is made by a person, for their own scripts and integrations, and it reaches every community that person belongs to. A community with something confidential in it had no say in that. **Settings → Community → Authentication** now has **Allow personal API keys**, on to begin with and switched by the security admin who writes the rest of that page. Turn it off and no key can be made for this community, a key someone already made no longer reaches it — the answer is decided when a key is used, not when it was made — and the cross-community views it would have appeared in leave it out. Keys are not cancelled: they keep working for every other community, and switching this back on gives people theirs back rather than leaving them to make replacements. It needs nothing from your operator, because it only ever narrows what reaches you — the **Authentication** tab is there for every community now, rather than only the ones an operator has granted their own sign-in, with those controls still on it where they have been granted. Nothing else changes: the app on a phone, single sign-on and the sessions people are already in are untouched.
- **A community can ask that you signed in with a second factor** — a community could insist on a particular sign-in provider, but not that the person behind it had presented a code. **Settings → Community → Authentication** now has **Also require a second factor**, alongside the provider choice and the community's own single sign-on; ask for one, the other, or both. It is answered by a code from an authenticator app and by a recovery code alike, because the set exists to be used. Nobody is signed out to satisfy it: a member already signed in is asked for a code where they are, and the session they are in carries on with everything it had already proved — satisfying one community's rule never un-satisfies another's. Two things have to be true before the rule saves, both the same shape as the provider rule beside it: the deployment offers the authenticator app, and the person writing the rule has set one up, so it is proved before it binds anybody.
- **A deployment chooses whether it offers the authenticator app** — two-factor is something people turn on for themselves, and an organisation that keeps that decision somewhere else had no way to say so. The ways-in checklist in **Settings → Platform → Authentication** has a third tick for it, alongside the password and single sign-on. Withdrawing it strands nobody — it is nobody's only way in — so it asks for no acknowledgement: what it does is stop the code being asked for at sign-in and stop new setups, while leaving the ones already made alone, so ticking it again asks for the code once more rather than making everybody start over. The Security page on each account says the site does not offer it rather than showing a button that would be refused. One rule changed with it: the checklist has always insisted on keeping at least one tick, and now insists on keeping at least one that can *begin* a sign-in, because a code goes alongside a password rather than instead of one. Deployments that upgrade offer it, exactly as a new install does.
- **Two-factor authentication** — a password was the only thing standing between anybody and an account. **User settings → Security** now sets one up with an authenticator app, and from then on signing in asks for your password and then a six-digit code — in a browser and in the app both, because a second factor that guards one door is not guarding much. Turning it on hands you ten **recovery codes**, shown once and good for one sign-in each; the page keeps count and says so when you are running low. Lost the phone and the codes with it? Whoever runs your deployment can clear the factor so you can set it up again — clear it, and nothing more: no screen anywhere shows somebody's key or their codes. Turning it off asks for your password and one more code, and signs you out on your other devices. Nobody is asked for anything until they turn it on.
- **A community can insist on its own sign-in without naming which provider** — a community's requirement could only ever name one identity provider, so a community running two (staff on one, contractors on the other) could not express the rule it actually wanted: come in through one of ours, we don't mind which. The requirement now offers **Any of our sign-in providers** alongside the named ones. It means what it says and no more: a sign-in through one of that community's own providers opens the door, and signing in elsewhere on the same deployment — including through the operator's own Google or Microsoft — does not. A password does not, and neither does a phone or an integration key, which record nothing about how their owner signed in. Requirements that name a single provider are unchanged, and a community with no requirement is untouched. As with naming a provider, you can only set it once your own session has come in that way, so the rule is proved before it binds anybody.
- **A deployment can decide it does not do direct messages** — My Messages was simply there, on every install, and an organisation that keeps private conversation somewhere else had no way to say so. **Settings → Platform → Community** now has a switch for it, alongside the community directory and the age question, and it belongs to the platform owner like the rest of that page. Turn it off and My Messages goes — from the sidebar, the bottom bar, the command palette and the **Privacy** tab of everyone's settings — the **Message** and **Connect** buttons come off profiles and member lists, and My Contacts, which lists the people you could reach out to, goes quiet with it. Nothing is deleted: conversations, connections, requests and everyone's own privacy settings stay exactly as they were, so turning it back on gives people their threads back rather than a fresh start. Messages are end-to-end encrypted and always were, so this is a decision about whether the feature is offered here, not a way to see inside it. Deployments that upgrade keep messaging on.
- **A direct message can have more than two people on it** — a conversation was always a pair, so sorting one thing out between four people meant four threads and repeating yourself in each. **Start a conversation** now lets you gather several people and start a group with them. Everybody named on it is *asked* rather than added: they see who else is on it before they answer, and nothing reaches them until they say yes — so a group is never somewhere you find yourself without agreeing to it. Everyone in a group has to be able to message everyone else, and you are told which two people cannot while you are still choosing names rather than after you commit. A group has no name: it is the people on it, which is also how you find it again, and proposing the same people twice opens the thread you already have rather than a second one. There is no owner and nobody can remove anybody — you can leave, and somebody who leaves or declines can be asked again later, because a no today is not a no forever. Nothing is kept for you while you are deciding, so when you do say yes somebody else's device sends you what was said in the meantime — as much of it as they still have. Messages are end-to-end encrypted exactly as they are between two people, and the bell and your phone name the thread by who is on it.
- **A sign-in rule says which provider it trusts** — group rules matched on the claim value alone, so two single-sign-on providers read each other's rules: whichever one somebody signed in through granted its own communities and then **removed the ones the other had granted**, and signing in through the other put them back and took the first lot away. A rule now belongs to one provider, named when you write it, and a sign-in reads only that provider's rules and only ever reclaims what that provider gave. Existing rules stay with the provider you already had. The groups claim moved to the provider it describes, in **Settings → Platform → Authentication**, so each one can spell groups its own way — and the background re-check now covers every provider instead of only the first.
- **An assistant can see what your work is connected to** — the Relations panel records what a task is blocked by, what a document was written from and what belongs to what, and an assistant connected over MCP could read none of it: it saw a task's status, its people and its tags, and nothing about the work either side of it. Asking what something is linked to is now part of that connection, answered the way the panel draws it — from that thing's side, with what each link points at — and narrowable to one kind of link or one kind of thing. It can draw one, too — say that a task is blocked by another, or that the document behind it belongs to the same project — which it asks you to confirm like any other change it makes. Removing a link, and replacing a thing's links wholesale, stay out: unlinking is done in the app.
- **Making an initiative walks you through it** — the old form put name, colour, joining and tools on one screen, with the tools folded into an accordion marked *Advanced*, so the decision that actually shapes an initiative was the one nobody opened. It is four questions now: what it is called, what it is made of, who can get in, and what an ordinary member can do once they are in. The tools step is a grid of the tools themselves rather than a column of switches — you can see what a queue or a counter is before deciding you want one — and it will not let you build an initiative with nothing in it. The last question is the one nothing used to ask: a tool you switch on is visible to managers and to nobody else until a role is given it, which is a strange thing to find out a week later. Creating a community now drops you straight into the wizard.
- **Report something, and a community settles it** — there was no way to tell anyone that something here was wrong. **Report** now sits on a comment, a notice, a picture, a profile and a community's listing in the directory, and where it goes is decided for you: something a community holds reaches that community's own moderators, while something about an account reaches whoever runs the deployment. You pick what is wrong, add anything you want them to know, and that is the end of your part — you are not told who reads it or what they decide, because a report is not a conversation. Two people reporting the same thing is one thing for a moderator to decide, not two. Moderators get a **Moderation** entry at the top of their initiative in the sidebar, which also gathers the initiative's members and a read-only view of how widely each thing in it is shared — visible only to them, and impossible to switch off, so there is never a community with nowhere for reports to land. Each one shows what was reported, why, how many people reported it and what they said, with a link to the thing itself; it closes as dismissed, content removed, member warned, or escalated to the deployment's moderators, and settled reports stay so "what did we do about this" has an answer. **Private conversations are not moderated and carry no Report action.**
- **Every setting you keep for a community, in one panel** — the operator's Guilds tab was a row of editors: a box for the member cap, a box for storage, a switch for sign-in, a switch for help requests, all squeezed into table columns. It had run out of room — the banner-artwork entitlement shipped with no control at all, because there was nowhere to put it. Each community now has **Manage**, which opens its settings grouped into Limits, Sign-in and Features, and the banner entitlement is in there with the rest. The table goes back to being a table: who they are, how many members against their cap, how much storage they're allowed, and their status.
- **Choose which ways in your deployment permits** — a password and single sign-on were both simply on, and the only way to run an SSO-only deployment was not to tell anyone the password form was there. **Settings → Platform → Authentication** now opens with the ways in, one tick each, and you keep whichever you want. At least one stays on. Where accounts sign in only one way, the page says how many before you withdraw it and asks you to confirm that number, so it is a decision made with the figure in front of you rather than a surprise afterwards — and nobody is signed out either way, on the web or on a phone. Withdrawing the password closes its sign-in, its registration and its reset together; withdrawing single sign-on empties the provider buttons, and is held back while any community still requires a sign-in through a provider of its own, naming how many. Existing deployments permit both, exactly as before.
- **Ask for help, from the sidebar** — the question mark in the bottom corner used to open the documentation front page. It now says **Ask for help**, and it opens the FAQ, which is where most of what people ask is already answered. Where whoever runs your deployment has said they are taking help requests, it opens a form instead: what it is about, what happened, sent. It is there for everyone in a community, not only its admins — the person who needs help is rarely the person who administers anything — and the FAQ stays one click away inside the form. Self-hosted installs are unaffected unless somebody turns it on: with nobody staffing a queue, the FAQ is the honest answer.
- **A comment shows you what it will look like before you post it** — comments and task descriptions have always been markdown, and the only way to check the syntax was to post it and see. Both now sit in the same composer: **Write** and **Preview** tabs, and a row of buttons for the syntax nobody remembers — headings, bold, italic, quote, code, links and the three kinds of list — folded into a menu when the screen is too narrow for them — with ⌘B, ⌘I and ⌘E from the keyboard. A list carries itself on: Enter under a bullet, a number or a checkbox makes the next one, and Enter on an empty item ends the list. A comment's toolbar carries **@** and **#** too, opening the same pickers the typed triggers do. The task editor's old **Edit**/**Preview** button is gone with it: the description is always a field now.
- **The front door is for the person who was sent a link** — the landing page talked to gamers about parties and quests, which is not who arrives at it: most people land there because their group already lives here and somebody invited them. It now leads with signing in and with what happens next — open the link, have a look round, you cannot break this — in the help center's voice, and starting a community of your own comes second. It shows what each tool is for, links the help center throughout, and has a **Get the app** section that puts your own device first: an Android download that matches the server you are on, and the home-screen install for everything else. Where a deployment sells hosted plans it shows them, with their current prices, a **Pricing** link in the header and a link to the full comparison; running it yourself and the enterprise plan get a band each, above and below the plans that compare against one another. A self-hosted install shows no plans at all.

### Changed

- **Letting a community run its own sign-in is two decisions, not one** — a single switch granted a community both the ability to connect its own identity provider *and* the ability to insist every member arrive through it. Those are different things, and an operator had no way to allow the first without the second. They are now two ticks in that community's **Sign-in** section: **Its own sign-in providers**, and **Requiring a sign-in**. A community that had the old switch on keeps both, so nothing changes for anyone. Withdrawing either still only closes the page it governs — their providers stay, their members keep signing in, and a requirement already set stays in force. Their admins can always lift a requirement whatever you've granted, so a community is never stuck behind a sign-in nobody can undo.
- **Deciding who may enter a community is the security admin's job** — a community's sign-in configuration, its identity providers and the requirement to arrive through one, was open to every admin it had. Running a community and deciding who may enter it are different jobs, and the security admin seat exists for the second: it sits above admin, an operator seats the first one, and after that whoever holds it passes it on. That seat is now the one that holds a community's sign-in configuration: the **Authentication** tab is theirs, and an ordinary admin no longer sees it — a page whose every control belongs to somebody else is not worth a tab. Everything else they ran, they still run. Where a community does require a sign-in, its last security admin stays put: that seat cannot be demoted, removed, or left behind while the requirement it governs is standing, so there is always somebody who can lift it. A community with nobody in the seat configures nothing until an operator seats someone, which is the shape intended for the many communities that never think about sign-in at all.
- **Sign-in is configured per community, and nothing gates it twice** — whether communities could configure their own sign-in used to be a deployment-wide either/or, fixed at install time, sitting on top of the per-community switch an operator already had. Two controls for one decision, and the deployment-wide one could not be changed without a redeploy. It is gone. The per-community switch in the platform's Guilds tab is now the whole of it: turn it on for a community and its admins configure their own sign-in, leave it off and they don't. Nothing changes for anyone who had it on.
- **Single sign-on works for everyone, everywhere** — an operator's own providers used to stop being offered the moment communities were allowed to configure theirs, so a deployment hosting many communities could offer a password and each community's provider and nothing else: no Google, no Microsoft, no Apple for anybody. They are offered always now. What a community requires is untouched, because a community's sign-in rule may only ever name a provider of its own — signing in with the operator's Google does not satisfy it.
- **A community's sign-in requirement can always be lifted** — reading a community's requirement, and clearing it, used to depend on whether that community was still entitled to set one. Both are now always available to that community's admins. Setting a requirement is unchanged and still needs the entitlement. Lifting one only ever admits more people, so it carries none of the conditions that setting one does.
- **A new project starts with three columns, not four** — every project arrived with **Backlog → In Progress → Blocked → Done**. Blocked was a hangover: what a task is waiting on is a relation now, recorded on the task itself, and parking it in a column said only that it had stopped. Backlog went too, because most people read it as To Do and filed everything there anyway. A new project now opens on **To Do → In Progress → Done**, with To Do wearing the plain slate circle and Backlog's dashed one still there for anyone who adds the column back. Projects that already exist keep exactly the columns they have, Blocked included, and the Backlog stage is still one of the four a status can belong to.
- **A status column can be deleted without a column to catch its tasks** — deleting a column with tasks in it demanded a replacement *in the same stage*, which a retired Blocked column usually had none of: the only way out was to invent a second To Do column, move everything by hand, and delete both. Now you pick any column you like, or leave it and the tasks fall to the project's default. The old rule that a project must keep its last Backlog and its last Done column has gone with it — what stages a project uses is its own business — and the one thing still refused is deleting a project's only status, because a board needs somewhere to put a task.
...
- **The roles screen shows what you can actually change** — Project Manager used to render sixteen permission switches that were pinned on and refused every click, because a manager holds everything by construction; it now says that in a sentence, the way Moderator already did. The rest is one row per tool with its View and Create together, rather than sixteen separate lines, and projects and documents sit with the other six instead of above a section headed *Tools* — they are tools. Granting Create now grants View with it, since making something you cannot open was never a state worth being able to save. Tools the initiative has switched off are gathered below a line under one explanation, instead of repeating the same caveat beside each of them.
...
- **Document and spreadsheet toolbars fit the pane, not the window** — the document swapped between a full toolbar and a stripped-down one at a screen width, so a document beside the sidebar got the narrow one on a wide monitor; the spreadsheet's wrapped into two and three rows, eating the rows it exists to show. Both are now one row that measures the width it actually has, shedding controls one at a time into a **⋯** menu that names them. A group of controls opens inside that menu rather than flying out beside it, so it is never cut off on a phone.

### Fixed

- **A community's name no longer runs under its member count** — on a community with no banner artwork, the banner is a short colour band, and the member and online counts floated in its corner above the words. A long enough name at a narrow enough width wrapped straight underneath them. The counts now sit in a row of their own at the top of the banner and the name starts below where they end, so there is no width, and no length of name, that puts one on top of the other.
- **A community admin can leave a community** — leaving was refused with "you are the last admin" for every admin, however many others the community had. The check counted the community's admins through the caller's own membership, which only ever finds the one. It counts the whole roster now, so an admin leaving a community that has others simply leaves, and the **Leave** button stops insisting otherwise. A genuine last admin is still held back — promote somebody first — and a security admin counts as an admin on both sides of that question: their being there frees an ordinary admin to go, and they are held in turn once they are the only one left.
- **Every direct message tells you it arrived** — a conversation announced its first message and then went silent: a reply never reached your phone, the bell kept counting up without ever clearing, and no device you were signed in on knew you had read anything. Two things were wrong. Your phone now buzzes for **every** message, the way a conversation needs it to, rather than once per conversation — the notification settings and quiet hours still decide whether it does. And reading a thread now clears its bell line everywhere you are signed in, instead of leaving a count that climbed forever. Email is unchanged and still arrives once, when a conversation has something waiting: it is the channel for when you are not there at all. Leaving a conversation takes its bell line down with it.
- **Restoring something from the trash brings back what is archived inside it** — restoring an initiative (or a project, or a document) was refused with "The thing this is inside is archived or in the trash" whenever anything inside it had been archived before it was deleted. Comments, queue items, counters, pictures and calendar events cannot be archived on their own — they are simply read-only inside an archived thing — and the database was refusing to let them out of the trash while that archived thing was still archived. The whole tree comes back now, with whatever was archived still archived. Something whose parent really is in the trash is still held back, and archived content is still read-only.
...
- **Opening a task in another project from the one you were on** — following a relation to a task in a different project left the Status field blank, then refused to let you leave the page without saving, and refused the save for want of a status. The field was reading the beat between the new task arriving and its project's columns arriving as somebody emptying it. A list that has not caught up yet clears nothing, on that field or any other.
