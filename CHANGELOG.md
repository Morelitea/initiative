...
- **Serve HTTPS without a reverse proxy** — point `TLS_CERT_FILE` and `TLS_KEY_FILE` at a certificate and its key and Initiative serves HTTPS itself, so a deployment that already has a certificate no longer has to stand up Caddy or nginx purely to hold it. Set `APP_URL` to the matching `https://` address; set one of the pair without the other and startup stops and names the half that is missing. The certificate is read at startup, so a renewed one takes effect on the next restart, and the key must be unencrypted. A proxy is still the easier road on the public internet, since it renews certificates for you.
