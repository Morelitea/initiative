...
- **Pause notifications until a date** — email and mobile stand down for an hour, the rest of the day, until Monday, or a date you choose. The bell keeps collecting throughout, and when the pause ends you get one email with everything that happened, minus whatever you read in the meantime. Nothing gets through while it is on, mentions included. Password resets and sign-in mail are unaffected, because those are answers rather than notifications.
