...
- **Import from Atlassian!**
  - **Bring your projects over from Jira** — the import wizard on **Community settings → Data** gains a **Jira** option that reads your site directly: connect with your site address, email and an API token, tick the projects, and pick the initiative they land in. Each project arrives with its columns, tasks, fields as properties, links between issues, sub-tasks under their parents, comments and images, and each sprint as an event on a calendar named after its board. Before anything is written you see what's coming — untick any field you'd rather not add as a property — and what isn't — a comment only some people could see, a link to an issue you didn't pick, sprints when the initiative has no calendars — and say who its people are here. Reading a big project takes a few minutes; close the window and opening the import again picks it back up. The token is used only to read your projects and is deleted as soon as they've been read.
  - **Bring your spaces over from Confluence** — the import wizard on **Community settings → Data** gains a **Confluence** option that reads your site the same way Jira does: connect with your site address, email and an API token, tick the spaces, and pick the initiative. Each space arrives as a wiki with its page tree intact, its home page, and labels as tags; folders, and parent pages with nothing of their own to say, become a list of the pages under them. Links between pages point at the imported pages, and the people who wrote and are mentioned in them go through the same "who is who" step as a Jira import. Before anything is written you see what is coming and what isn't — macros with no equivalent here, like a table of contents or a draw.io diagram, are named and counted. Attached files don't come over yet. Jira issues a page mentions — through Confluence's Jira macro or a plain link — point at the tasks they became when the Jira project was imported first, with a smart chip showing each task's live status, and status lozenges come across as statuses in their own colours.
