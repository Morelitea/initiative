...
- **A deployment can show arriving visitors a cookie notice** — a short line along the bottom of the page the first time somebody lands, saying what Initiative keeps in their browser and why, with a link to the detail and to the deployment's own privacy policy where it has one. It is off until a platform owner turns it on under **Settings → Platform → Branding**, which suits a server with a public front door and leaves one a group reaches by invitation alone. It explains rather than asks — everything kept is needed to run what somebody came for, so dismissing it changes nothing — and it appears once per browser. Where an administrator has configured a sign-up spam check, the notice names the company behind it.
