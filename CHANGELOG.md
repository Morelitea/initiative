...
- **Breaking glass asks for your authenticator code** — reaching a community you do not belong to is the most powerful thing anyone can do here, and it was protected by nothing the person doing it was not already carrying. Now, as soon as **anybody** who can break glass has set up two-factor authentication, breaking glass asks everybody for a code at the moment they do it — a recovery code as readily as a live one, because the phone is the thing most likely to be missing in the hour somebody needs this. Nobody switches it on: it follows from who has enrolled, so it can only ever be asking while at least one person can answer it, and there is no arrangement that leaves a deployment nobody can break glass into. The other side of that is worth knowing: if a colleague enrols and you have not, your next break-glass is refused until you do, and it says so and points at the page. Deployments where nobody has enrolled are untouched, and the request-and-approve path, where a second person is already the control, is unchanged.
