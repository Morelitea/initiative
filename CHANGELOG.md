...
- **Two-factor authentication** — a password was the only thing standing between anybody and an account. **User settings → Security** now sets up an authenticator app: confirm your password, scan the square (or type the key underneath it, for a desktop app or a camera that has seen better days), and enter the six digits it shows. From then on signing in asks for your password and then a code — in a browser and in the app both, because a second factor that guards one door is not guarding much. Turning it on hands you ten **recovery codes**, shown once, each good for one sign-in, and the Security page keeps count of how many you have left and says so when you are running low; **New recovery codes** replaces the set. Lost the phone and the codes with it? Whoever runs your deployment can clear the factor so you can sign in and set it up again — clear it, and nothing more: there is no screen anywhere, at any level, that shows somebody's key or their codes. Turning it off asks for your password and one more code, discards the recovery codes, and signs you out on your other devices. Nobody is asked for anything until they choose to turn it on.
