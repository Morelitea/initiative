...
- **A community can decline personal API keys** — an API key is made by a person, for their own scripts and integrations, and it reaches every community that person belongs to. A community with something confidential in it had no say in that. **Settings → Community → Authentication** now has **Allow personal API keys**, on to begin with and switched by the superadmin who writes the rest of that page. Turn it off and no key can be made for this community, a key someone already made no longer reaches it — the answer is decided when a key is used, not when it was made — and the cross-community views it would have appeared in leave it out. Keys are not cancelled: they keep working for every other community, and switching this back on gives people theirs back rather than leaving them to make replacements. It needs nothing from your operator, because it only ever narrows what reaches you: the **Authentication** tab is the superadmin's, rather than only for the ones an operator has granted their own sign-in, and the controls an operator does grant are still on it where they have been. Nothing else changes: the app on a phone, single sign-on and the sessions people are already in are untouched.
...
- **Communities have a superadmin** — a community's sign-in configuration and its billing were open to every admin it had. Running a community day to day, and deciding who may get into it or what it is billed for, are different jobs, and there is now a seat for the second: **superadmin**, above admin. Whoever starts a community holds it, and can pass it to somebody else; every admin a community has today becomes one, so nobody loses anything they could do before — admins appointed from here on are ordinary admins. The **Authentication** tab and the billing portal are theirs, and an ordinary admin no longer sees either: a page whose every control belongs to somebody else is not worth a tab. Everything else admins ran, they still run. Every community keeps at least one superadmin — the last one cannot be demoted, removed, or leave, and cannot delete their account while they hold it — because only whoever runs the deployment can seat a community that has emptied it. That replaces the old "last admin" rule entirely: an ordinary admin can now leave a community, or close their account, whenever they like, because the community still has somebody who can appoint another.
