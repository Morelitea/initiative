...
- **Connecting single sign-on asks the provider instead of guessing** — setting one up meant eight fields on one screen, four of which are only knowable at the other end, and the first sign that any of them was wrong was somebody failing to sign in a week later, for a reason recorded only in a server log. **Add provider** in **Settings → Platform → Authentication**, and in a community's own, now walks through it in the order it actually happens: pick your provider, give its address, take the callback back to it, then the handful of options that already have sensible answers. Ten providers are there by name — Google, Entra ID, Okta, Auth0, Keycloak, Authentik, Authelia, Pocket ID, Zitadel, and anything else that speaks OpenID Connect — and each one asks for the parts of its address that differ rather than a URL to hand-edit, so a Keycloak realm and an Entra tenant are fields instead of something to paste into the middle of a link. **Verify** then goes and asks: if something answers and agrees it is the issuer you named, you get a tick and a list of what it found, and if it does not, you find out while you are still looking at the form. What it says it supports fills in the scopes, and the groups claim suggests what the provider actually carries while still taking a nested path it never mentions. Saved providers get **Test**, which asks the same question again — useful when somebody has been reorganising things at the far end — and the callback URL every row shows now has a copy button, it being the one value that has to travel from here to there. Editing stays a single form, with the same Verify and the same callback to hand.
- **A community can decline personal API keys** — an API key is made by a person, for their own scripts and integrations, and it reaches every community that person belongs to. A community with something confidential in it had no say in that. **Settings → Community → Authentication** now has **Allow personal API keys**, on to begin with and switched by the security admin who writes the rest of that page. Turn it off and no key can be made for this community, a key someone already made no longer reaches it — the answer is decided when a key is used, not when it was made — and the cross-community views it would have appeared in leave it out. Keys are not cancelled: they keep working for every other community, and switching this back on gives people theirs back rather than leaving them to make replacements. It needs nothing from your operator, because it only ever narrows what reaches you: the **Authentication** tab is now there for every community with somebody in the security admin seat, rather than only for the ones an operator has granted their own sign-in, and the controls an operator does grant are still on it where they have been. Nothing else changes: the app on a phone, single sign-on and the sessions people are already in are untouched.
