...
- **Optional S3-compatible object storage for uploads.** Uploads can now be stored in an S3-compatible object store (AWS S3, or self-hosted MinIO/R2/etc.) instead of the local filesystem, via `STORAGE_BACKEND=s3`. The filesystem remains the default, so existing deployments are unaffected. A `docker-compose.minio.yml` overlay and `docs/OBJECT_STORAGE.md` walk self-hosters through running it against MinIO; see also the new `S3_*` settings in `.env.example`.
