...
- **One name for who made something.** Every kind of guild content now records its author in a single field, `created_by` — replacing `author_id` on comments, `uploaded_by_id` on file versions, `installed_by_id` on installed apps, `created_by_user_id` on webhook subscriptions, and `created_by_id` everywhere it already existed. A document's `updated_by_id` is gone — nothing read it, and who last changed a document is already recorded with the change itself. API clients reading the old field names need updating.
