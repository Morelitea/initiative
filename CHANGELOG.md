...
- **Signing in to the app through a browser** — single sign-on and passkey sign-in in the app now count toward a community that asks for that sign-in, and they finish even when the phone closed the app while the browser was open. Signing in to the app with an emailed code now lasts past the next launch instead of asking again. Older versions of the app keep signing in as before until they update.
