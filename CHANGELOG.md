...
- **Galleries: a home for the pictures** — a new tool beside Documents and Posts for the visual half of the work: mockups, screenshots, app icons, exported designs. A gallery is a named wall of pictures with its own sharing, comments, tags and trash, and it is browsed three ways — a masonry that keeps every picture's shape, a grid of tiles, and a timeline with a row per day and a rail for jumping months — the first two groupable by tag. Drop a folder of files anywhere on the page and they go up a few at a time, each one checked from its bytes and thumbnailed for the wall; every picture can be titled, captioned and tagged, replaced by a new version with the earlier rounds kept as history, and opened full-size in a lightbox you can page through with the arrow keys or a swipe. Select several to tag or remove them together. A gallery's list card shows the cover you chose, or a grid of its four newest pictures. Off by default per initiative, like every non-core tool.
