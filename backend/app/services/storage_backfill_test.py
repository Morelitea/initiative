"""Tests for the DB-backed backfill runner (shared cross-worker status).

The ``storage_backfill_state`` row is RLS-forced and reachable only by the
``app_admin`` (BYPASSRLS) engine, so these drive it through ``role_session(
"app_admin")`` — the real production privilege boundary.
"""
...
async def _reset_backfill_row(admin_session):
    """Reset the singleton to idle before each test so they don't bleed."""
    from sqlalchemy import update

    from app.models.platform.storage_backfill_state import StorageBackfillState

    await storage_backfill._ensure_row(admin_session)
    await admin_session.execute(
        update(StorageBackfillState)
        .where(StorageBackfillState.id == storage_backfill.GLOBAL_ID)
        .values(status="idle", heartbeat=None, started_at=None, finished_at=None)
    )
...
async def test_stale_running_can_be_reclaimed(admin_session) -> None:
    ...
    await admin_session.execute(
        update(StorageBackfillState)
        .where(StorageBackfillState.id == storage_backfill.GLOBAL_ID)
        .values(heartbeat=storage_backfill._now() - timedelta(hours=2))
    )
    await admin_session.commit()
