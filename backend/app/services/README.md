...
|---|---|---|---|
| `tenant/` | per-guild content (`guild_<id>` schemas) | `RLSSessionDep` under `/c/{guild_id}/…` | documents, tasks (statuses), queues, counters, calendar_events, comments, initiatives, properties, project import/export, soft_delete, trash_purge, webhook dispatch/subscriptions |
...
| *root* (here) | **both sides** — cross-cutting infra | varies / called by both | `rls`, `membership`, `permissions`, `content_sockets`, `email`, `notifications`, `cross_guild`, `ai_settings`, `ai_generation`, `background_tasks`, `soft-delete filters`, `captcha`, `hibp`, … |
