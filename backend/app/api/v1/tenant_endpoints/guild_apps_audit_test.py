...
class TestInstalling:
    async def test_an_install_records_its_listing_and_how_it_arrived(
        self, client: AsyncClient, session: AsyncSession, acting_user, calendar_app
    ):
    ...
    async def test_a_refused_install_records_nothing(
        ...
        assert await recorded(session, AuditEventType.APP_INSTALLED) == []
...
class TestManaging:
    async def test_a_rename_records_which_fields_moved(
        ...
        (row,) = await recorded(session, AuditEventType.APP_UPDATED)
        assert row.actor_user_id == a.user.id
        assert row.guild_id == a.guild.id
        assert (row.target_type, row.target_id) == ("app", app["id"])
        ...
        assert "Club nights" not in str(row.envelope)
    ...
    async def test_a_patch_that_moves_nothing_records_nothing(
        ...
        assert await recorded(session, AuditEventType.APP_UPDATED) == []
    ...
    async def test_an_upgrade_records_the_two_versions(
        ...
        (row,) = await recorded(session, AuditEventType.APP_UPDATED)
        assert row.actor_user_id == a.user.id
        assert (row.target_type, row.target_id) == ("app", app_id)
        assert row.envelope["detail"] == {
            "area": "version",
            "from": "1.0.0",
            "to": "1.1.0",
...
class TestConfiguring:
    ...
    async def test_config_records_the_field_names_and_none_of_the_values(
        ...
        assert SECRET not in str(row.envelope)
        assert "example.test" not in str(row.envelope)
...
class TestUninstalling:
    async def test_removal_records_what_went_with_it(
        ...
        (row,) = await recorded(session, AuditEventType.APP_UNINSTALLED)
        assert row.actor_user_id == a.user.id
        assert row.guild_id == a.guild.id
        assert (row.target_type, row.target_id) == ("app", app["id"])
        assert row.envelope["detail"] == {
