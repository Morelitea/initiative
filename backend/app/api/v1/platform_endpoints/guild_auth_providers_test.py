...
async def _guild_superadmin(session: AsyncSession):
...
async def test_the_seat_has_full_crud(client: AsyncClient, session: AsyncSession):
    admin, guild = await _guild_superadmin(session)
...
async def test_member_and_non_member_denied(client: AsyncClient, session: AsyncSession):
    _admin, guild = await _guild_superadmin(session)
...
async def test_slug_unique_per_guild_not_across_namespaces(
    ...
    admin_a, guild_a = await _guild_superadmin(session)
    admin_b, guild_b = await _guild_superadmin(session)
...
async def test_other_namespace_rows_unreachable(
    ...
    admin, guild = await _guild_superadmin(session)
    _admin_b, guild_b = await _guild_superadmin(session)
