...
from app.testing import TOOL_FACTORIES, route_session_to_guild
...
async def test_exporting_a_roster_is_recorded_with_its_count(
    ...
    rows = await recorded(session, AuditEventType.GUILD_MEMBERS_EXPORTED)
...
async def test_an_export_that_matched_nobody_records_nothing(
    ...
    assert await recorded(session, AuditEventType.GUILD_MEMBERS_EXPORTED) == []
...
async def test_removing_a_member_is_recorded_against_the_admin_who_did_it(
    ...
    assert [_where(row) for row in rows] == [
        (admin_id, member_id, guild_id, "guild", guild_id)
    ]
    assert rows[0].envelope["detail"] == {"role": "member", "via": "admin"}
...
async def test_removing_someone_who_is_not_a_member_records_nothing(
    ...
    assert response.status_code == 404
    assert await recorded(session, AuditEventType.GUILD_MEMBER_REMOVED) == []
...
async def test_a_transfer_records_one_move_counted_by_tool(
    ...
    rows = await recorded(session, AuditEventType.CONTENT_OWNERSHIP_TRANSFERRED)
    assert [_where(row) for row in rows] == [
        (admin.user.id, member.user.id, admin.guild.id, None, None)
    ]
...
async def test_a_claim_records_the_move_with_no_previous_owner(
    ...
    assert [_where(row) for row in rows] == [
        ...
        (admin.user.id, None, admin.guild.id, None, None)
    ]
    detail = rows[0].envelope["detail"]
...
async def test_a_transfer_that_moved_nothing_records_nothing(
    ...
    assert await recorded(session, AuditEventType.CONTENT_OWNERSHIP_TRANSFERRED) == []
