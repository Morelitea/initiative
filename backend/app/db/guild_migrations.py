...
2. **New guilds** — provisioning builds a fresh guild schema by running
   ...
   (``alembic upgrade head`` then ``python scripts/gen_guild_schema.py`` — it
   reflects ``guild_template`` — → commit) so guilds created later include the
   change.

The **drift-guard test** (``schema_provisioning_test``) provisions a fresh
schema and diffs it against ``guild_template``: if you applied a delta to
existing schemas but forgot to regenerate the artifact (or vice-versa), a new
