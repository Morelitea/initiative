...
def test_guild_name_builders_reject_or_sanitize_hostile_ids(builder, hostile):
    ...
    assert re.fullmatch(r"[A-Za-z0-9_]*guild_[0-9]+(_ro|_support|_q|_app)?", name), name
...
def test_guild_name_builders_emit_identifier_safe_names(builder):
    ...
    an optional ``_ro``/``_support``/``_q``/``_app`` suffix — no quotable characters."""
    name = builder(42)
    assert re.fullmatch(r"[A-Za-z0-9_]*guild_42(_ro|_support|_q|_app)?", name), name
