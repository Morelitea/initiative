...
DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'ck_initiative_role_permissions_permission_key' AND connamespace = current_schema()::regnamespace) THEN ALTER TABLE "initiative_role_permissions" ADD CONSTRAINT "ck_initiative_role_permissions_permission_key" CHECK (((permission_key)::text = ANY (ARRAY[('docs_enabled'::character varying)::text, ('projects_enabled'::character varying)::text, ('create_docs'::character varying)::text, ('create_projects'::character varying)::text, ('queues_enabled'::character varying)::text, ('create_queues'::character varying)::text, ('events_enabled'::character varying)::text, ('create_events'::character varying)::text, ('advanced_tool_enabled'::character varying)::text, ('create_advanced_tool'::character varying)::text, ('counters_enabled'::character varying)::text, ('create_counters'::character varying)::text]))); END IF; END $$;
