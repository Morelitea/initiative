    ...
    <div className="space-y-6">
      {generatedSecret ? (
        <Card className="border-primary/50 shadow-sm">
          <CardHeader>
            ...
            <CardDescription>Copy this key now. You will not be able to view it again.</CardDescription>
          </CardHeader>
      ...
      <Card className="shadow-sm">
        <CardHeader>
          ...
          <CardDescription>Create long-lived credentials for scripts or integrations.</CardDescription>
        </CardHeader>
        <CardContent>
      ...
      <Card className="shadow-sm">
        <CardHeader>
          ...
          <CardDescription>Only the last few characters are shown. Rotate keys regularly.</CardDescription>
        </CardHeader>
        <CardContent>
                    ...
                    return (
                      <tr key={key.id} className="border-t">
                        <td className="py-3 pr-4">
                          ...
                          <div className="text-xs text-muted-foreground">{formatDateTime(key.created_at)}</div>
                        </td>
                        <td className="py-3 pr-4 font-mono">{key.token_prefix}•••</td>
                        <td className="py-3 pr-4">
                          <Badge variant={status.variant}>{status.label}</Badge>
                        </td>
                        <td className="py-3 pr-4">{key.last_used_at ? formatDateTime(key.last_used_at) : 'Never'}</td>
                        ...
                        <td className="py-3 text-right">
                          ...
                          >
                            {deleteTarget === key.id && deleteKey.isPending ? 'Removing…' : 'Delete'}
                          </Button>
                        </td>
                      </tr>
                    );
