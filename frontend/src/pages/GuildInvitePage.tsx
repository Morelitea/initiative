...
export const GuildInvitePage = () => {
  ...
  const inviteTitle = inviteValid
    ? `Join ${status?.guild_name ?? "this guild"}`
    : "Guild invite";
          ...
          {loading ? (
            ...
            <>
              <div className="rounded border bg-muted/40 p-4 text-sm">
                ...
                {status?.max_uses ? (
                  <p>
                    <span className="font-medium">Uses:</span> {status.uses ?? 0} / {status.max_uses}
                  </p>
                ) : null}
              </div>
              {inviteValid ? (
                <div className="space-y-2 text-sm text-muted-foreground">
                  ...
                  <p>
                    Need an account?{" "}
                    <Link className="text-primary underline-offset-4 hover:underline" to={registerLink}>
