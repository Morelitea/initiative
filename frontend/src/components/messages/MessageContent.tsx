 ...
 * * **Pictures.** An image tag would have the reader's browser fetch a URL the
 *   sender chose, the moment it arrived — which tells the sender where the
 *   reader is and when they opened it, from inside a conversation whose whole
 *   point is that nobody learns either. So an image becomes a link with its
 *   name on it, and goes nowhere until somebody decides it should.
 *
 * Raw HTML is not enabled, so a body cannot bring markup of its own, and
 * react-markdown drops a link whose scheme is not one it will follow.
 */
const PROSE =
