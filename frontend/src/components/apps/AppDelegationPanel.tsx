 ...
 *
 * The app-wide question draws only for an app that actually acts as people.
 * Whether it does is the operator's grant on the registration, not something
 * the install claims, so an app that loses the grant stops asking.
 *
 * Beside it, one line for each purpose the app has asked to act as you for
 * (`AppConsentRequests`), each answered on its own.
 */
...
import type { GuildAppConsentRead } from "@/api/generated/initiativeAPI.schemas";
...
export interface AppDelegationPanelProps {
  ...
  /** Whether the app-wide question is asked at all. */
  delegates?: boolean;
  /** The viewer's own answers to the app's requests, one per purpose. */
  consents?: GuildAppConsentRead[];
}

export function AppDelegationPanel({
  appId,
  appName,
  delegation,
  delegates = true,
  consents = [],
}: AppDelegationPanelProps) {
  ...
  return (
    <section className="space-y-3 rounded-lg border p-4">
      ...
      {consents.length > 0 && (
        <AppConsentRequests appId={appId} appName={appName} consents={consents} />
      )}
    </section>
  );
}
