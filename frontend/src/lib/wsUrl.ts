...
/**
 * Build the ws/wss URL for a guild-scoped WebSocket endpoint from the API
 * base (which may be relative in dev, where Vite proxies it). `subpath` is
...
export const buildGuildWsUrl = (guildId: number, subpath: string): string => {
  ...
  const normalizedPath = url.pathname.endsWith("/")
    ...
    : url.pathname || "/api/v1";
  url.pathname = `${normalizedPath}/g/${guildId}/${subpath}`;
  url.search = "";
  url.hash = "";
  return url.toString();
};
